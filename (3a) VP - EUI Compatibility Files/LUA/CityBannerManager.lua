		...
		if city then
			...
			-- Set left banner icon to city state flag
				if originalCityOwner:IsMinorCiv() then
					...
					if (Game.IsOption("GAMEOPTION_ENABLE_VASSALAGE") and not Game.IsOption("GAMEOPTION_NO_VOLUNTARY_VASSALAGE") and not Players[originalCityOwnerID]:IsAlive() and Teams[Players[Game.GetActivePlayer()]:GetTeam()]:GetCurrentEra() >= Game.GetVassalageEnabledEra() and Players[Game.GetActivePlayer()]:CanLiberatePlayer(originalCityOwnerID)) then
