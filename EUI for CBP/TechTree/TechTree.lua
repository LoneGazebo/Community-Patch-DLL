...
local function TechSelected( techID )
	if techID > -1 then
		if g_stealingTechTargetPlayer then
			if Teams[ g_stealingTechTargetPlayer:GetTeam() ]:IsHasTech( techID ) then
...
local function RefreshDisplayOfSpecificTech( tech )
	...
	local turnText = Locale.ConvertTextKey( "TXT_KEY_STR_TURNS", g_activePlayer:GetResearchTurnsLeft( techID, true ) )
	...
	local showAlreadyResearched, showFreeTech, showCurrentlyResearching, showAvailable, showUnavailable, showLocked, queueUpdate, queueText, turnLabel, setCallback
...
function( popupInfo )
	...
	if g_popupInfoType then
		...
		UIManager:QueuePopup( ContextPtr, isKey and PopupPriority.InGameUtmost or PopupPriority.TechTree )
