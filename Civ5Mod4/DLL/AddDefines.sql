...
INSERT INTO Defines (
Name, Value)
SELECT 'INFLUENCE_TARGET_DISTANCE_WEIGHT_VALUE', '1';
...
-- Warmonger Base Values
UPDATE Defines
Set Value = -8
WHERE Name = 'WARMONGER_THREAT_PER_TURN_DECAY' AND EXISTS (SELECT * FROM CSD WHERE Type='CSD_AI' AND Value= 1 );

UPDATE Defines
Set Value = 225
WHERE Name = 'WARMONGER_THREAT_MINOR_ATTACKED_WEIGHT' AND EXISTS (SELECT * FROM CSD WHERE Type='CSD_AI' AND Value= 1 );

UPDATE Defines
Set Value = 900
WHERE Name = 'WARMONGER_THREAT_MINOR_CONQUERED_WEIGHT' AND EXISTS (SELECT * FROM CSD WHERE Type='CSD_AI' AND Value= 1 );

UPDATE Defines
Set Value = 225
WHERE Name = 'WARMONGER_THREAT_MAJOR_ATTACKED_WEIGHT' AND EXISTS (SELECT * FROM CSD WHERE Type='CSD_AI' AND Value= 1 );

UPDATE Defines
Set Value = 900
WHERE Name = 'WARMONGER_THREAT_MAJOR_CONQUERED_WEIGHT' AND EXISTS (SELECT * FROM CSD WHERE Type='CSD_AI' AND Value= 1 );
