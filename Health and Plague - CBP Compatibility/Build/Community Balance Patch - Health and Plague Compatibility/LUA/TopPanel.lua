print("This is the modded TopPanel from CBP- H&P")
-- PLAGUE MOD
	...
	if( iPlayerID >= 0 ) then
		...
		if (pPlayer:GetNumCities() > 0) then
			...
			if (Game.IsOption(GameOptionTypes.GAMEOPTION_NO_RELIGION)) then
				...
				strFaithStr = string.format("%i (+%i)", pPlayer:GetFaith(), (pPlayer:GetTotalFaithPerTurn() + pPlayer:GetFaithFromSpecialists())); --MOD
			...
			-----------------------------
			local strHealthStr;
			local iHealth = pPlayer:GetCivHealthIndex()
			if iHealth < 0 then
...
function DoInitTooltips()
	...
	Controls.HealthString:SetToolTipCallback( HealthTipHandler ); --PLAGUE MOD
...
function FaithTipHandler( control )
	...
	if (Game.IsOption(GameOptionTypes.GAMEOPTION_NO_RELIGION)) then
		...
		local iFaithFromSpecialists = pPlayer:GetFaithFromSpecialists();
		if (iFaithFromSpecialists ~= 0) then
			strText = strText .. "[NEWLINE]";
			strText = strText .. Locale.ConvertTextKey("TXT_KEY_TP_FAITH_FROM_SPECIALISTS", iFaithFromSpecialists);
		end
		--end MOD
...
function HealthTipHandler( control )
	...
	local iCivHealthIndex, iTraitModifier, iTechModifier, iPolicyModifier, iTradeModifier, iUnHappyPenalty, iDifficultyPenalty = pPlayer:GetCivHealthIndex()

	strText = strText .. Locale.ConvertTextKey("TXT_KEY_TP_HEALTH_OVERALL") .. ConvertTextColor(iCivHealthIndex);
	strText = strText .. "[NEWLINE]";

	--Trait modifier
	if (iTraitModifier ~= 0) then
		strText = strText .. "[NEWLINE]";
		strText = strText .. "[ICON_BULLET]" .. ConvertTextColor(iTraitModifier) .. Locale.ConvertTextKey("TXT_KEY_TP_HEALTH_TRAIT");
	end

	--Policy modifier
	if (iPolicyModifier ~= 0) then
		strText = strText .. "[NEWLINE]";
		strText = strText .. "[ICON_BULLET]" .. ConvertTextColor(iPolicyModifier) .. Locale.ConvertTextKey("TXT_KEY_TP_HEALTH_POLICY");
	end

	--Tech modifier
	if (iTechModifier ~= 0) then
		strText = strText .. "[NEWLINE]";
		strText = strText .. "[ICON_BULLET]" .. ConvertTextColor(iTechModifier) .. Locale.ConvertTextKey("TXT_KEY_TP_HEALTH_TECH");
	end

	--Trade modifier
	if (iTradeModifier ~= 0) then
		strText = strText .. "[NEWLINE]";
		strText = strText .. "[ICON_BULLET]" .. ConvertTextColor(iTradeModifier) .. Locale.ConvertTextKey("TXT_KEY_TP_HEALTH_TRADE");
	end

	--Happiness modifier
	if (iUnHappyPenalty ~= 0) then
		strText = strText .. "[NEWLINE]";
		if iUnHappyPenalty < 0 then
			strText = strText .. "[ICON_BULLET]" .. ConvertTextColor(iUnHappyPenalty) .. Locale.ConvertTextKey("TXT_KEY_TP_HEALTH_UNHAPPY");
		else
			strText = strText .. "[ICON_BULLET]" .. ConvertTextColor(iUnHappyPenalty) .. Locale.ConvertTextKey("TXT_KEY_TP_HEALTH_GOLDENAGE");
		end
	end

	--Difficulty modifier
	if (iDifficultyPenalty < 0) then
		strText = strText .. "[NEWLINE]";
		strText = strText .. "[ICON_BULLET]" .. ConvertTextColor(iDifficultyPenalty) .. Locale.ConvertTextKey("TXT_KEY_TP_HEALTH_DIFFICULTY");
	end
...
function ConvertTextColor(iValue)
	local strValue;
	if (iValue < 0) then
		strValue = Locale.ConvertTextKey("TXT_KEY_TP_HEALTH_NEGATIVE", iValue);
	else
		strValue = Locale.ConvertTextKey("TXT_KEY_TP_HEALTH_POSITIVE", iValue);
	end
	return strValue;
end
--end addition
