...
int CvGrandStrategyAI::GetCulturePriority()
{
	...
	int iFlavorGrowth =  m_pPlayer->GetFlavorManager()->GetPersonalityIndividualFlavor((FlavorTypes)GC.getInfoTypeForString("FLAVOR_GROWTH"));
	int iFlavorExpansion =  m_pPlayer->GetFlavorManager()->GetPersonalityIndividualFlavor((FlavorTypes)GC.getInfoTypeForString("FLAVOR_EXPANSION"));

	iPriority += (iFlavorGrowth - iFlavorExpansion) * GC.getAI_GS_CULTURE_MAX_CITIES_MODIFIER();
...
int CvGrandStrategyAI::GetUnitedNationsPriority()
{
	...
	//antonjs: consider: change this 750 gold heuristic to reflect larger gold costs in Civ5XP
	...
	//antonjs: consider: Wary Of penalties
...
int CvGrandStrategyAI::GetSpaceshipPriority()
{
	...
	iPriority += m_pPlayer->GetCurrentEra() * (iFlavorScience);
