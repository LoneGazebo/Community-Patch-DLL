...
void CvTechAI::Read(FDataStream& kStream)
{
	...
	{
		kStream >> iWeight;
		m_TechAIWeights.push_back(i, iWeight);
	}
}
...
TechTypes CvTechAI::ChooseNextTech()
{
...
void CvTechAI::LogPossibleResearch()
{
	if (GC.getLogging() && GC.getAILogging())
	{
		...
		for (int iI = 0; iI < m_ResearchableTechs.size(); iI++)
		{
			...
			strTemp.Format ("%s, %d", GC.getTechInfo(eTech)->GetType(), iWeight);
...
void CvTechAI::LogResearchChoice(TechTypes eTech)
{
	if (GC.getLogging() && GC.getAILogging())
	{
		...
		strTemp.Format ("CHOSEN, %s", GC.getTechInfo(eTech)->GetType());
