...
function UpdateScreen()
    ...
    for pCity in pPlayer:Cities() do
		...
		local iTotalUnhappyCP = (iCultureUnhappiness + iDefenseUnhappiness + iGoldUnhappiness + iConnectionUnhappiness + iPillagedUnhappiness + iStarvingUnhappiness + iMinorityUnhappiness) * 100;
...
-- COMMUNITY CHANGE
		...
		if(pCity:IsCapital()) then
			if (iCityCountMod ~= 0) then
				fUnhappinessTimes100 = (iTotalUnhappyCP / 100);
			end
		end
--END
...
-- COMMUNITY CHANGE
		...
		if (iConnectionUnhappiness ~= 0) then
			strOccupationTT = strOccupationTT .. "[NEWLINE][NEWLINE]" .. Locale.ConvertTextKey("TXT_KEY_CONNECTION_UNHAPPINESS", iConnectionUnhappiness);
		end

		-- Pillaged tooltip
		if (iPillagedUnhappiness ~= 0) then
			strOccupationTT = strOccupationTT .. "[NEWLINE][NEWLINE]" .. Locale.ConvertTextKey("TXT_KEY_PILLAGED_UNHAPPINESS", iPillagedUnhappiness);
		end

		-- Starving tooltip
		if (iStarvingUnhappiness ~= 0) then
			strOccupationTT = strOccupationTT .. "[NEWLINE][NEWLINE]" .. Locale.ConvertTextKey("TXT_KEY_STARVING_UNHAPPINESS", iStarvingUnhappiness);
		end

		-- Minority tooltip
		if (iMinorityUnhappiness ~= 0) then
			strOccupationTT = strOccupationTT .. "[NEWLINE][NEWLINE]" .. Locale.ConvertTextKey("TXT_KEY_MINORITY_UNHAPPINESS", iMinorityUnhappiness);
		end
