...
local preacherTexture = "citizenPreacher.dds"; --MOD
local doctorTexture = "citizenUnemployed.dds"; --MOD
...
function AddBuildingButton( pCity, building )
	...
	if (pCity:IsHasBuilding(buildingID)) and building.IsVisible then --PLAGUE MOD support for compatibility
		...
		if building.SpecialistType then
			...
			for pYieldInfo in GameInfo.Yields() do
				...
				if pYieldInfo.Type == healthYieldType then
					iYieldAmount = GetHealthFromSpecialist(building.SpecialistType);
				else
					iYieldAmount = pCity:GetSpecialistYield(iSpecialistID, iYieldID);
				end
				--END
			...
			elseif building.SpecialistType == "SPECIALIST_PREACHER" then --MOD
				controlTable.BuildingFilledSpecialistSlot1:SetTexture(preacherTexture);
				controlTable.BuildingFilledSpecialistSlot2:SetTexture(preacherTexture);
				controlTable.BuildingFilledSpecialistSlot3:SetTexture(preacherTexture);
			elseif building.SpecialistType == "SPECIALIST_DOCTOR" then --MOD
				controlTable.BuildingFilledSpecialistSlot1:SetTexture(doctorTexture);
				controlTable.BuildingFilledSpecialistSlot2:SetTexture(doctorTexture);
				controlTable.BuildingFilledSpecialistSlot3:SetTexture(doctorTexture);
			else
...
function OnCityViewUpdate()
	...
	if (pCity ~= nil) then
		...
		if IsPlagued(pCity) then
			Controls.PlagueIcon:SetHide(false);
			Controls.PlagueIcon:LocalizeAndSetToolTip("TXT_KEY_PM_CITY_PLAGUED");
		else
...
--		UpdateSpecialists(pCity);
		...
		local iCityHealthIndex = GetCityHealthIndex(pCity);
		if iCityHealthIndex < 0 then
			Controls.CityHealthLabel:SetText( Locale.ConvertTextKey("TXT_KEY_CITYVIEW_PERTURN_TEXT_NEGATIVE", iCityHealthIndex) );
		else
			Controls.CityHealthLabel:SetText( Locale.ConvertTextKey("TXT_KEY_CITYVIEW_PERTURN_TEXT", iCityHealthIndex) );
		end
		--end addition
		...
		if (Game.IsOption(GameOptionTypes.GAMEOPTION_NO_RELIGION)) then
			...
			local iFaithPerTurn = pCity:GetFaithPerTurn() + City_GetFaithFromSpecialists(pCity); --MOD
...
function DoUpdateUpperLeftTooltips()
	...
	--FAITH specialist mod
	local strFaithToolTip;
	if g_Properties.FaithSpecialist then
		strFaithToolTip = GetMODFaithTooltip(pCity);
	else
		strFaithToolTip = GetFaithTooltip(pCity);
	end
	Controls.FaithBox:SetToolTipString(strFaithToolTip);
	--end

	--PLAGUE MOD
	local strHealthToolTip = GetHealthTooltip(pCity, false);
