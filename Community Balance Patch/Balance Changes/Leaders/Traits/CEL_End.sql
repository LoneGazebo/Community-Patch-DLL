...
UPDATE Units SET	   Cost = 100   WHERE Type IN ('UNIT_HUN_BATTERING_RAM');
					   
UPDATE Units SET	   Cost = 190   WHERE Type IN ('UNIT_CARTHAGINIAN_FOREST_ELEPHANT');
UPDATE Units SET	   Cost = 240   WHERE Type IN ('UNIT_SONGHAI_MUSLIMCAVALRY');
...
UPDATE Units SET	   Cost   = 360  WHERE Type IN ('UNIT_FRENCH_FOREIGNLEGION');

UPDATE Units SET	   Cost   = ROUND(0.80 * (SELECT Cost   FROM Units WHERE Type = 'UNIT_PIKEMAN'), 0)
					   WHERE Type IN ('UNIT_GERMAN_LANDSKNECHT');

					   -- Based on next unit
UPDATE Units SET	   Cost   = 850	   WHERE Type IN ('UNIT_GERMAN_PANZER');
...
UPDATE Units SET	   Cost   = 280   WHERE Type IN ('UNIT_JAPANESE_SAMURAI');
UPDATE Units SET	   Combat = 28    WHERE Type IN ('UNIT_JAPANESE_SAMURAI');
					   
UPDATE Units SET	   Cost   = 280   WHERE Type IN ('UNIT_SPANISH_CONQUISTADOR');
UPDATE Units SET	   Cost   = 250   WHERE Type IN ('UNIT_SPANISH_TERCIO');

UPDATE Units SET	   Cost   = 260   WHERE Type IN ('UNIT_KOREAN_HWACHA');
...
UPDATE Units SET RangedCombat = ROUND(1.11 * (SELECT RangedCombat FROM Units WHERE Type = 'UNIT_CAVALRY'), 0)
					   WHERE Type IN ('UNIT_RUSSIAN_COSSACK');

UPDATE Units SET	   Combat = ROUND(1.05 * (SELECT Combat FROM Units WHERE Type = 'UNIT_LANCER'), 0)
					   WHERE Type IN ('UNIT_AUSTRIAN_HUSSAR');
UPDATE Units SET	   Cost   = ROUND(1.00 * (SELECT Cost   FROM Units WHERE Type = 'UNIT_LANCER'), 0)
					   WHERE Type IN ('UNIT_AUSTRIAN_HUSSAR');
UPDATE Units SET RangedCombat = ROUND(0.00 * (SELECT RangedCombat FROM Units WHERE Type = 'UNIT_CAVALRY'), 0)
					   WHERE Type IN ('UNIT_AUSTRIAN_HUSSAR');
					   
UPDATE Units SET	   Combat = ROUND(0.77 * (SELECT Combat FROM Units WHERE Type = 'UNIT_KNIGHT'), 0)
					   WHERE Type IN ('UNIT_MONGOLIAN_KESHIK');
UPDATE Units SET RangedCombat = ROUND(0.81 * (SELECT Combat FROM Units WHERE Type = 'UNIT_KNIGHT'), 0)
					   WHERE Type IN ('UNIT_MONGOLIAN_KESHIK');
UPDATE Units SET	   Cost   = ROUND(0.81 * (SELECT Cost   FROM Units WHERE Type = 'UNIT_KNIGHT'), 0)
					   WHERE Type IN ('UNIT_MONGOLIAN_KESHIK');
					   
UPDATE Units SET	   Combat = ROUND(0.81 * (SELECT Combat FROM Units WHERE Type = 'UNIT_KNIGHT'), 0)
...
UPDATE Buildings SET	Cost   = ROUND(0.77 * (SELECT Cost   FROM Buildings WHERE Type = 'BUILDING_TEMPLE'), 0)
						WHERE Type IN ('BUILDING_BASILICA');
