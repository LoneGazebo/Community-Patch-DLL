...
std::vector<ScoreCityEntry> CvEspionageAI::BuildDefenseCityList()
{
	...
	for(pLoopCity = GET_PLAYER(ePlayer).firstCity(&iLoop); pLoopCity != NULL; pLoopCity = GET_PLAYER(ePlayer).nextCity(&iLoop))
	{
		...
		if (MOD_BALANCE_CORE_SPIES_ADVANCED)
		{
			...
			{
				int iDiff = GC.getESPIONAGE_GATHERING_INTEL_COST_PERCENT() - iResistance;
				iDiff /= 10;
				iValue -= iDiff;
			}
		}
		...
		if (pLoopCity->GetEspionageRanking() > 6 || iResistance > GC.getESPIONAGE_GATHERING_INTEL_COST_PERCENT())
		{
			iValue *= 75;
			iValue /= 100;
		}
