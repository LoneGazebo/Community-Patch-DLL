...
CvAIOperation* CreateAIOperation(AIOperationTypes eAIOperationType, int iID, PlayerTypes eOwner, PlayerTypes eEnemy)
{
	...
	case AI_OPERATION_ESCORTED_IMPROVEMENT_BUILD:
		return new CvAIOperationBuildImprovementEscorted(iID, eOwner, -1, SBuilderDirective(), false, false);
	}
...
void CvAIOperationBuildImprovementEscorted::Init(CvCity* /*pTarget*/, CvCity* /*pMuster*/)
{
	...
	//don't wait for the escort in the wild
	CvPlot* pMusterPlot = pBuilder->plot();
	if (IsEscorted() && pMusterPlot->getOwner() != m_eOwner)
	{
		CvCity* pClosestCity = NULL;
		if (IsNavalOperation())
			pClosestCity = OperationalAIHelpers::GetClosestFriendlyCoastalCity(m_eOwner, pBuilder->plot(), 1);
		else if (!pMusterPlot->IsFriendlyTerritory(m_eOwner))
			pClosestCity = GET_PLAYER(m_eOwner).GetClosestCityByPathLength(pBuilder->plot());
	...
	//we know the civilian is slot 0
	pArmy->AddUnit(pBuilder->GetID(), 0, true);

	//AddUnit may have upgraded the unit, in which case slot 0 now holds a different unit than we started with
	CvArmyFormationSlot* pBuilderSlot = pArmy->GetSlotStatus(0);
	if (pBuilderSlot && pBuilderSlot->IsUsed())
	{
		m_iBuilderID = pBuilderSlot->GetUnitID();
		pBuilder = GetBuilder();
		if (!pBuilder)
		{
			SetToAbort(AI_ABORT_LOST_CIVILIAN);
	...
	//a little hack - set the escort slot to not required if we can reach the target this turn
	if (IsKill() && pTargetSite && pBuilder->TurnsToReachTarget(pTargetSite, CvUnit::MOVEFLAG_TURN_END_IS_NEXT_TURN, 1) < 1)
	{
...
MultiunitFormationTypes CvAIOperationBuildImprovementEscorted::GetFormationType() const
{
	...
	switch (pBuilder->getUnitInfo().GetDefaultUnitAIType())
	{
...
CvPlot* CvAIOperationBuildImprovementEscorted::GetBuilderTargetPlot() const
{
	...
	CvCity* pClosestCity = GET_PLAYER(pBuilder->getOwner()).GetClosestCityByPathLength(pBuilder->plot());
	if (pClosestCity)
...
CvUnit* CvAIOperationBuildImprovementEscorted::GetBuilder() const
{
	return m_iBuilderID != -1 ? GET_PLAYER(m_eOwner).getUnit(m_iBuilderID) : NULL;
}
...
CvAIOperationBuildImprovementEscorted* OperationalAIHelpers::GetBuilderEscortOperation(PlayerTypes ePlayer, const CvUnit* pUnit)
{
	if (ePlayer == NO_PLAYER || !pUnit || pUnit->getArmyID() == -1)
		return NULL;

	CvArmyAI* pArmy = GET_PLAYER(ePlayer).getArmyAI(pUnit->getArmyID());
	CvAIOperation* pOperation = pArmy ? pArmy->GetOperation() : NULL;
	if (!pOperation || pOperation->GetOperationType() != AI_OPERATION_ESCORTED_IMPROVEMENT_BUILD)
		return NULL;

	return dynamic_cast<CvAIOperationBuildImprovementEscorted*>(pOperation);
}
...
bool CvAIOperationBuildImprovementEscorted::PreconditionsAreMet(CvPlot*, CvPlot*, int)
{
	...
	vector<CvArmyFormationSlot> fakeStatus;
	CvMultiUnitFormationInfo* thisFormation = GC.getMultiUnitFormationInfo(GetFormationType());
	if (!thisFormation || thisFormation->getNumFormationSlotEntries() == 0)
		return false;
	...
	fakeStatus[freeSlots[0].first] = CvArmyFormationSlot(pBuilder->GetID(), freeSlots[0].second.m_requiredSlot);
