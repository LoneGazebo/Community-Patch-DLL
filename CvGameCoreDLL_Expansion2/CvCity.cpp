...
void CvCity::UpdateCityYields(YieldTypes eYield)
{
	...
	{
		SetStaticYield(eYield, getJONSCulturePerTurn(false));
	}
	else if(eYield == YIELD_FAITH)
	{
		SetStaticYield(eYield, GetFaithPerTurn(false));
	}
	else
	{
...
#endif
{
	...
	if(bStatic)
	{
		return GetStaticYield(YIELD_CULTURE);
	}
#endif
...
#endif
{
	...
	if(bStatic)
	{
		return (GetStaticYield(YIELD_FAITH));
	}
#endif
	...
	if(eReligionFounded != NO_RELIGION)
	{
		...
		if(pReligion)
		{
			if(pReligion->m_Beliefs.GetYieldFromKnownPantheons(eYield) > 0)
			{
				...
				if(iPantheon > 0)
				{
					int iYieldValue = (iPantheon * pReligion->m_Beliefs.GetYieldFromKnownPantheons(eYield));
...
void CvCity::DoBarbIncursion()
{
	...
	if (MOD_DIPLOMACY_CITYSTATES_QUESTS && GET_PLAYER(getOwner()).isBarbarian())
	{
