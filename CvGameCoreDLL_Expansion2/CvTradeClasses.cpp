...
bool CvGameTrade::CreateTradeRoute(CvCity* pOriginCity, CvCity* pDestCity, DomainTypes eDomain, TradeConnectionType eConnectionType, int& iRouteID)
{
	...
	//TRs turn over more quickly the long the game goes on.
	int iCurrentEra = GET_PLAYER(pOriginCity->getOwner()).GetCurrentEra();
	iTargetTurns -= (iCurrentEra * 2);
#else
		...
		if (pTradeConnection->m_eOriginOwner == m_pPlayer->GetID())
		{
			...
			if (!pTrade->IsTradeRouteIndexEmpty(ui) && pTradeConnection->m_iCircuitsCompleted >= pTradeConnection->m_iCircuitsToComplete)
			{
				...
				if(pTradeConnection->m_eConnectionType == TRADE_CONNECTION_INTERNATIONAL)
				{
					...
					float fDelay = 0.0f;
					int iEra = GET_PLAYER(pTradeConnection->m_eOriginOwner).GetCurrentEra();
					if(iEra < 1)
					{
						iEra = 1;
					}
							...
							switch(iYield)
							{
								...
								case YIELD_GOLD:
									...
									if(pTradeConnection->m_eOriginOwner == GC.getGame().getActivePlayer())
									{
										char text[256] = {0};
										fDelay += 0.5f;
										sprintf_s(text, "[COLOR_YELLOW]+%d[ENDCOLOR][ICON_GOLD]", iYieldFromStartingRoute * iEra);
										DLLUI->AddPopupText(iOriginX, iOriginY, text, fDelay);
									}
									break;
								case YIELD_GREAT_GENERAL_POINTS:
									...
									if(pkBuildingInfo && pkBuildingInfo->GetCorporationID() == GET_PLAYER(pOriginCity->getOwner()).GetCorporateFounderID())
									{
										for(int iYield = 0; iYield < NUM_YIELD_TYPES; iYield++)
										{
											if(pkBuildingInfo->GetCorporationTradeRouteMod(iYield) > 0)
											{
												...
												if(pOriginCity->GetCityBuildings()->GetNumBuilding(eBuilding) > 0)
												{
													iScore *= (pkBuildingInfo->GetCorporationTradeRouteMod(iYield) * 5);
												}
											}
										}
									}
...
int CvTradeAI::ScoreFoodTR (const TradeConnection& kTradeConnection, CvCity* pSmallestCity)
{
	...
	int iDistance = kTradeConnection.m_aPlotList.size();
#if defined(MOD_BALANCE_CORE)
	int iDistanceScore = MAX(0, iMaxDistance - iDistance);
#else
...
#if defined(MOD_BALANCE_CORE)
	int iScore = 0;
	if(m_pPlayer->getCapitalCity() != NULL && pSmallestCity != NULL)
	{
		iScore = (m_pPlayer->getCapitalCity()->getPopulation() - pSmallestCity->getPopulation());
		if(pSmallestCity->getUnhappinessFromConnection() > 0 || pSmallestCity->getUnhappinessFromStarving() > 0)
		{
			iScore *= 2;
		}
		for (int iYieldLoop = 0; iYieldLoop < NUM_YIELD_TYPES; iYieldLoop++)
	...
	return (iScore - (iDistanceScore + iDangerSum));
#else
...
int CvTradeAI::ScoreProductionTR (const TradeConnection& kTradeConnection, std::vector<CvCity*> aTargetCityList)
{
	...
	{
		CvCity* pDestCity = pPlot->getPlotCity();
		if(pDestCity != NULL && m_pPlayer->getCapitalCity() != NULL)
		{
			iScore = (m_pPlayer->getCapitalCity()->getBaseYieldRate(YIELD_PRODUCTION) - pDestCity->getBaseYieldRate(YIELD_PRODUCTION));
			if(pDestCity->getUnhappinessFromConnection() > 0)
			{
				iScore *= 2;
			}
			for (int iYieldLoop = 0; iYieldLoop < NUM_YIELD_TYPES; iYieldLoop++)
