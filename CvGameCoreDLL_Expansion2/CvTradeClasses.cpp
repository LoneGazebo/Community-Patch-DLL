...
#endif
{
	...
	if (!m_pPlayer->isBarbarian() && (!m_pPlayer->GetPlayerTraits()->IsCanPlunderWithoutWar() || (!GET_TEAM(m_pPlayer->getTeam()).isAtWar(GET_PLAYER(eOwningPlayer).getTeam()) && m_pPlayer->GetPlayerTraits()->IsCanPlunderWithoutWar() && pPlunderPlot->isVisible(eOwningTeam))))
	{
...
{	
	...
	if (MOD_BALANCE_CORE_GOLD_INTERNAL_TRADE_ROUTES)
	{
		if (aGoldInternalSortedTR.size() > 0)
		{
			aTradeConnectionList.push_back(aGoldInternalSortedTR[0].m_kTradeConnection);
		}
	}
#endif
