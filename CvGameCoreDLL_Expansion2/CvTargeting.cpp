...
// Same as above functionality but with the addition that any plot that is not
// revealed, and has no revealed neighbor, is always considered to have minimum
// elevation. Used by exploration AI to figure out where we could potentially
// see plots.
// 
//	---------------------------------------------------------------------------
//	Test if the team can see from the supplied hex to a displacement in HEXSPACE
...
//	after a certain distance threshold is reached.  Even when below the distance threshold
//	more than one blockage will cause the LOS to fail.
static bool CanMaybeSeeDisplacementPlot_Strict(int startX, int startY, int dx, int dy, int fromLevel, TeamTypes eTeam)
{
	int originalDX = dx;
	int originalDY = dy;
...
//	from the start hex.
//	This is the 'loose' version that lets you see around partially blocked hexes
static bool CanMaybeSeeDisplacementPlot_Loose(int startX, int startY, int dx, int dy, int fromLevel, TeamTypes eTeam)
{
	int originalDX = dx;
	int originalDY = dy;
...
bool CvTargeting::CanMaybeSeeDisplacementPlot(int startX, int startY, int dx, int dy, int fromLevel, TeamTypes eTeam)
{
	// convert the start coord to hex-space coordinates
	// No need to convert the Y coord, it is the same in both systems
	startX = xToHexspaceX(startX, startY);
	...
	if (pPlot != NULL)
	{
		...
		// If over STRICT_LOOSE_CUTOFF away, any blockage past the STRICT_LOOSE_CUTOFF will result in not being able to target.
		if (iHexDistanceBetweenPlots > STRICT_LOOSE_CUTOFF /*&& GC.getGame().isOption("STRICT_RANGE_TARGETING")*/)
			return CanMaybeSeeDisplacementPlot_Strict(startX, startY, dx, dy, fromLevel, eTeam);
		else
			return CanMaybeSeeDisplacementPlot_Loose(startX, startY, dx, dy, fromLevel, eTeam);
