...
bool CvMinorCivAI::IsEnabledQuest(MinorCivQuestTypes eQuest, EraTypes eEra, bool bCheckPriorEras, bool bRecursive)
{
	if (!bRecursive)
	{
		...
		case MINOR_CIV_QUEST_SPY_ON_MAJOR:
			...
			break;
		case MINOR_CIV_QUEST_COUP:
			bInvalid = MOD_BALANCE_VP || GC.getGame().isOption(GAMEOPTION_NO_ESPIONAGE);
			break;
		default:
			break;
		}
	...
	for (int iEraLoop = 0; iEraLoop < GC.getNumEraInfos(); iEraLoop++)
	{
		...
		if (IsEnabledQuest(eQuest, eLoopEra, /*bCheckPriorEras*/ false, /*bRecursive*/ true))
			return true;
	}
...
bool CvMinorCivAI::IsSameStateReligionAsMajor(PlayerTypes eMajor)
{
	...
	if (pCapital)
	{
		...
		if (eMajorReligion == NO_RELIGION)
			eMajorReligion = GET_PLAYER(eMajor).GetReligions()->GetStateReligion(false);
