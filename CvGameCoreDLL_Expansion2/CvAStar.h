...
typedef int(*CvAGetExtraChildren)(const CvAStarNode*, const CvAStar*, std::vector<std::pair<int,int>>&);
...
public:
	...
	int GetExtraChildren(const CvAStarNode* node, std::vector<std::pair<int,int>>& out) const;
...
public:
	...
	virtual std::map<CvPlot*,SPath> GetMultiplePaths(const CvPlot* pStartPlot, std::vector<CvPlot*> vDestPlots, const SPathFinderUserData& data);
