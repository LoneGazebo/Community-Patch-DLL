...
int CvDealAI::GetThirdPartyPeaceValue(bool bFromMe, PlayerTypes eOtherPlayer, TeamTypes eWithTeam)
{
	...
	if(bFromMe)
	{
		...
		if(iWarScore > 75)
			iItemValue += 500;
		else if(iWarScore > 50)
			iItemValue += 300;
		else if(iWarScore > 25)
			iItemValue += 200;
		else if(iWarScore <= 25)
			iItemValue += 100;
		else
	...
	else
	{
		...
		if(iTheirWarScore > 75)
			iItemValue += 500;
		else if(iTheirWarScore > 50)
			iItemValue += 300;
		else if(iTheirWarScore > 25)
			iItemValue += 200;
		else if(iTheirWarScore <= 25)
			iItemValue += 100;
		else
		...
		if(!GET_PLAYER(eWithPlayer).isMinorCiv())
		{
			...
			else if(eOpinionTowardsWarPlayer == MAJOR_CIV_OPINION_UNFORGIVABLE)
			{
				iItemValue *= 25;
				iItemValue /= 100;
			}
...
int CvDealAI::GetTechValue(TechTypes eTech, bool bFromMe, PlayerTypes eOtherPlayer)
{
	...
	if(bFromMe)
	{
		...
		if(GetPlayer()->GetDiplomacyAI()->GetMajorCivOpinion(eOtherPlayer) < MAJOR_CIV_OPINION_FRIEND)
		{
			return INT_MAX;
		}
#endif
