...
int CvDealAI::GetResourceValue(ResourceTypes eResource, int iResourceQuantity, int iNumTurns, bool bFromMe, PlayerTypes eOtherPlayer)
{
	...
	else if(eUsage == RESOURCEUSAGE_STRATEGIC)
	{
		...
		if(!GET_TEAM(GetPlayer()->getTeam()).IsResourceObsolete(eResource))
		{
			...
			iItemValue *= max(1, iResourceQuantity);
			// Opinion also matters
			int iModifier = 0;
			switch(GetPlayer()->GetDiplomacyAI()->GetMajorCivOpinion(eOtherPlayer))
			{
	...
	if(bFromMe)
	{
		...
		if (MOD_BALANCE_CORE_DEALS) 
		{
			iModifier += 115;	// Not forced value
		}
#else
...
int CvDealAI::GetOpenBordersValue(bool bFromMe, PlayerTypes eOtherPlayer, bool bUseEvenValue)
{
	...
	if(bFromMe)
	{
		...
		{
#if defined(MOD_BALANCE_CORE_DEALS)
			iItemValue *= (100 - (iNumEnemiesAtWarWith * 10));
#else
			iItemValue *= 10;
#endif
			iItemValue /= 100;
...
int CvDealAI::GetThirdPartyPeaceValue(bool bFromMe, PlayerTypes eOtherPlayer, TeamTypes eWithTeam)
{
...
#if defined(MOD_BALANCE_CORE)
	int iItemValue = 25 + GC.getGame().getGameTurn(); //just some base value
#else
	int iItemValue = 0;
#endif
	...
	else
	{
		...
		if(!GET_PLAYER(eWithPlayer).isMinorCiv())
		{
			// Modify for our feelings towards the player we're dealing with
...
bool CvDealAI::IsMakeOfferForCity(PlayerTypes eOtherPlayer, CvDeal* pDeal, bool bBuying)
{
	CvAssert(eOtherPlayer >= 0);
	CvAssert(eOtherPlayer < MAX_MAJOR_CIVS);
	...
	int iBestCity = 0;
	// Logic for when THIS AI wants to buy a city. Only applies to cities close to our borders.
	if(bBuying)
	{
		for(pLoopCity = GET_PLAYER(eOtherPlayer).firstCity(&iCityLoop); pLoopCity != NULL; pLoopCity = GET_PLAYER(eOtherPlayer).nextCity(&iCityLoop))
		{
	...
	{
		// Logic for when THIS AI wants to sell a city. Only applies to cities close to them
		for(pLoopCity = GET_PLAYER(eOtherPlayer).firstCity(&iCityLoop); pLoopCity != NULL; pLoopCity = GET_PLAYER(eOtherPlayer).nextCity(&iCityLoop))
		{
			if(pLoopCity != NULL)
			{
	...
	{
		if(pBestCity->getOwner() == m_pPlayer->GetID())
		{
			pDeal->AddCityTrade(m_pPlayer->GetID(), pBestCity->GetID());
		}
		else if(pBestCity->getOwner() == eOtherPlayer)
		{
...
bool CvDealAI::IsMakeOfferForCityExchange(PlayerTypes eOtherPlayer, CvDeal* pDeal)
{
	...
	if(pBestBuyCity != NULL)
	{
		...
		for(pLoopCity = GET_PLAYER(eOtherPlayer).firstCity(&iCityLoop2); pLoopCity != NULL; pLoopCity = GET_PLAYER(eOtherPlayer).nextCity(&iCityLoop2))
		{
			if(pLoopCity != NULL)
			{
				...
				{
					//Don't wnat to compare distance for city we're about to trade
					if(pLoopMyCity != NULL && pBestSellCity != pLoopMyCity)
					{
						iTempDistance = plotDistance(pLoopMyCity->getX(), pLoopMyCity->getY(), pLoopCity->getX(), pLoopCity->getY());
						//Only trade border cities, okay?
						if(iTempDistance <= (GC.getAI_DIPLO_PLOT_RANGE_FROM_CITY_HOME_FRONT() * 2))
	...
	if(pBestBuyCity->getOwner() == m_pPlayer->GetID())
	{
		pDeal->AddCityTrade(m_pPlayer->GetID(), pBestBuyCity->GetID());
	}
	else
	{
		return false;
	}
	if(pBestSellCity->getOwner() == eOtherPlayer)
	{
		pDeal->AddCityTrade(eOtherPlayer, pBestSellCity->GetID());
	}
	else
	{
		return false;
...
bool CvDealAI::IsMakeOfferForThirdPartyWar(PlayerTypes eOtherPlayer, CvDeal* pDeal)
{
	...
	for (uint ui = 0; ui < MAX_CIV_PLAYERS; ui++)
	{
		PlayerTypes eAgainstPlayer = (PlayerTypes)ui;
		...
		//Minor civ? Only if we're not protective.
		if(GET_PLAYER(eAgainstPlayer).isMinorCiv() && ((GetPlayer()->GetDiplomacyAI()->GetMinorCivApproach(eAgainstPlayer) == MINOR_CIV_APPROACH_FRIENDLY) || (GetPlayer()->GetDiplomacyAI()->GetMinorCivApproach(eAgainstPlayer) == MINOR_CIV_APPROACH_PROTECTIVE)))
		{
			continue;
		}
		//not alive?
		...
		//Can we declare war?
		if(!GET_TEAM(m_pPlayer->getTeam()).canDeclareWar(GET_PLAYER(eAgainstPlayer).getTeam()))
		{
			continue;
		}
		...
		if(GetPlayer()->GetDiplomacyAI()->GetMajorCivApproach(eAgainstPlayer, false) == MAJOR_CIV_APPROACH_FRIENDLY)
		{
			continue;
		}
		// Can we actually complete this deal?
		if(pDeal->IsPossibleToTradeItem(eOtherPlayer, GetPlayer()->GetID(), TRADE_ITEM_THIRD_PARTY_WAR, eWithTeam))
		{
			...
			if((iWarValue < 100000) && (iWarValue > -100000) && iWarValue > iBestValue)
			{
...
bool CvDealAI::IsMakeOfferForThirdPartyPeace(PlayerTypes eOtherPlayer, CvDeal* pDeal)
{
	...
	for (uint ui = 0; ui < MAX_CIV_PLAYERS; ui++)
	{
		...
		//Minor civ? Only if we're protective.
		if(GET_PLAYER(eAgainstPlayer).isMinorCiv() && ((GetPlayer()->GetDiplomacyAI()->GetMinorCivApproach(eAgainstPlayer) != MINOR_CIV_APPROACH_FRIENDLY) || (GetPlayer()->GetDiplomacyAI()->GetMinorCivApproach(eAgainstPlayer) != MINOR_CIV_APPROACH_PROTECTIVE)))
		{
			continue;
		}
		...
		//Can we declare war?
		if(!GET_TEAM(m_pPlayer->getTeam()).canChangeWarPeace(GET_PLAYER(eAgainstPlayer).getTeam()))
		{
			continue;
		}
		//Can they declare war?
		if(!GET_TEAM(eWithTeam).canChangeWarPeace(GET_PLAYER(eAgainstPlayer).getTeam()))
		{
			continue;
		}
		//Is our opinion of the player good? Don't do it!
		...
		//Is our approach not friendly? Don't do it!
		if(GetPlayer()->GetDiplomacyAI()->GetMajorCivApproach(eAgainstPlayer, false) < MAJOR_CIV_APPROACH_AFRAID)
		{
			continue;
		}
		// Can we actually complete this deal?
		if(pDeal->IsPossibleToTradeItem(eOtherPlayer, GetPlayer()->GetID(), TRADE_ITEM_THIRD_PARTY_PEACE, eWithTeam))
		{
