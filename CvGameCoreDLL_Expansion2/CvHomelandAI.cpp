...
void CvHomelandAI::RecruitUnits()
{
	...
	for(CvUnit* pLoopUnit = m_pPlayer->firstUnit(&iLoop); pLoopUnit != NULL; pLoopUnit = m_pPlayer->nextUnit(&iLoop))
	{
		...
		//don't touch units in armies - with one exception: the civilian of a builder escort, whose
		//moves we drive from ExecuteWorkerMoves. the military escort must stay out, otherwise the
		//garrison/sentry/patrol moves later in AssignHomelandMoves would pull it away from the civilian
		//and the army would drop it for falling behind.
		if (pLoopUnit->getArmyID() != -1)
		{
			CvAIOperationBuildImprovementEscorted* pBuildOperation = OperationalAIHelpers::GetBuilderEscortOperation(m_pPlayer->GetID(), pLoopUnit);
			if (!pBuildOperation || pBuildOperation->GetBuilder() != pLoopUnit)
			{
				pLoopUnit->SetTurnProcessed(true);
				continue;
			}
...
void CvHomelandAI::PlotOpportunisticSettlementMoves()
{
	...
	if (!bPerformMoves)
	{
		...
		for (list<int>::iterator it = m_CurrentTurnUnits.begin(); it != m_CurrentTurnUnits.end(); ++it)
		{
			...
			if (pUnit && eUnitAI != UNITAI_SETTLE && pUnit->IsCombatUnit() && pUnit->AI_getUnitAIType() == UNITAI_SETTLE)
			{
				pUnit->AI_setUnitAIType(eUnitAI);
				if (GC.getLogging() && GC.getAILogging())
				{
					CvString strLogString;
					strLogString.Format("Cancelling secondary settler %s (%d), X: %d, Y: %d", pUnit->getName().GetCString(), pUnit->GetID(), pUnit->getX(), pUnit->getY());
					LogHomelandMessage(strLogString);
				}
			}
		}
		...
		for (list<int>::iterator it = m_CurrentTurnUnits.begin(); it != m_CurrentTurnUnits.end(); ++it)
		{
			...
			if (pUnit && pUnit->IsCombatUnit() && (pUnit->isFound() || pUnit->IsFoundAbroad() || pUnit->IsFoundLate() || pUnit->IsFoundMid()))
			{
				...
				if ((pUnit->getExperienceTimes100() < 1500 || iUnitAge < 3) && pUnit->canUseForAIOperation())
				{
					...
					{
						CvString strLogString;
						strLogString.Format("Found secondary settler %s (%d), X: %d, Y: %d", pUnit->getName().GetCString(), pUnit->GetID(), pUnit->getX(), pUnit->getY());
						LogHomelandMessage(strLogString);
					}
				}
			}
	...
	if (m_CurrentMoveUnits.size() > 0)
	{
		if (m_CurrentMoveUnits.size() > 1)
		{
			CvPlot* pTargetPlot = m_pPlayer->GetBestSettlePlot(NULL);
			if (!pTargetPlot)
				pTargetPlot = m_pPlayer->getCapitalCity()->plot();

			for (CHomelandUnitArray::iterator it = m_CurrentMoveUnits.begin(); it != m_CurrentMoveUnits.end(); ++it)
			{
				CvUnit* pUnit = m_pPlayer->getUnit(it->GetID());
				int iDistance = pUnit ? plotDistance(pUnit->getX(), pUnit->getY(), pTargetPlot->getX(), pTargetPlot->getY()) : INT_MAX;
				it->SetAuxIntData(iDistance);
			}

			// Sort them by proximity to the best settle plot
...
void CvHomelandAI::PlanWorkerDistribution()
{
	...
	//no workers to distribute, or nowhere to put them (we may have lost our last city)
	if (iAvailableWorkers == 0 || aWorkerRegions.empty())
	{
...
{
	ClearCurrentMoveUnits(AI_HOMELAND_MOVE_WORKER);
	...
	for (list<int>::iterator it = m_CurrentTurnUnits.begin(); it != m_CurrentTurnUnits.end(); ++it)
	{
		...
		if (pUnit)
		{
			...
			if (bUsePrimaryUnit || bUseSecondaryUnit)
			{
				CvHomelandUnit unit;
				unit.SetID(pUnit->GetID());
				m_CurrentMoveUnits.push_back(unit);
...
static bool NeedsEscort(CvPlayer* pPlayer, CvUnit* pBuilder, const CvPlot* pPlot, const std::vector<SWorkerRegion>& workerRegions)
{
	...
	// Other units need an escort while they are not in the same worker region as the target (no worker region is NULL).
	// This means we have to travel into unsafe territory. Worker regions only ever contain plots we consider safe,
	// so once the builder has reached the target's region the escort has done its job and can be released.
...
CvAIOperationBuildImprovementEscorted* CvHomelandAI::AddBuildOperation(CvUnit* pBuilder, SBuilderDirective eDirective, bool bInstantBuild, bool bIsKill)
{
	...
	CvAIOperationBuildImprovementEscorted* pNewOperation = new CvAIOperationBuildImprovementEscorted(GC.getGame().GetNextGlobalID(), m_pPlayer->GetID(), pBuilder->GetID(), eDirective, bInstantBuild, bIsKill);
	if (!pNewOperation)
		return NULL;
...
bool CvHomelandAI::AddAndExecuteEscortedBuildOperation(CvUnit* pUnit, SBuilderDirective eDirective)
{
	...
	{
		//the unit may be in some other kind of army - in that case we must not touch it
		CvArmyAI* pArmy = GET_PLAYER(pUnit->getOwner()).getArmyAI(pUnit->getArmyID());
		pOperation = pArmy ? dynamic_cast<CvAIOperationBuildImprovementEscorted*>(pArmy->GetOperation()) : NULL;
		if (!pOperation)
			return false;
...
void CvHomelandAI::ExecuteWorkerMoves()
{
		...
		if (bCanBuild)
		{
			...
			if (bIsAutomated)
			{
				...
				else
				{
					...
					{
						CvArmyAI* pArmy = m_pPlayer->getArmyAI(pBuilder->getArmyID());
						CvAIOperation* pEscortOperation = pArmy ? pArmy->GetOperation() : NULL;
						//only ever tear down our own builder escorts, never some other operation the unit belongs to
						if (pEscortOperation && pEscortOperation->GetOperationType() == AI_OPERATION_ESCORTED_IMPROVEMENT_BUILD)
							pEscortOperation->Kill();
					}
	...
	for (list<int>::iterator it = allWorkers.begin(); it != allWorkers.end(); ++it)
	{
		...
		//this worker found no work this turn. if it is being escorted, retarget the operation at the city so the
		//escort comes along - walking there on our own would leave it behind. the operation will not do this by
		//itself: it has no retargeting logic, so without a new directive it would keep heading for the build
		//target it was last given, which by now may well have been taken by somebody else.
		CvAIOperationBuildImprovementEscorted* pEscortOperation = OperationalAIHelpers::GetBuilderEscortOperation(m_pPlayer->GetID(), pUnit);
		if (pEscortOperation)
		{
			if (pBestCity && AddAndExecuteEscortOperation(pUnit, pBestCity->plot()))
			{
				mapCityAssignedWorkers[pBestCity->GetID()]++;
				continue;
			}

			//the operation cannot continue - release the units and handle the worker on its own below
			pEscortOperation->Kill();
		}
...
bool CvHomelandAI::IsValidExplorerEndTurnPlot(const CvUnit* pUnit, CvPlot* pPlot) const
{
	...
	//don't target barbarian camps that we don't have vision of
	if (pPlot->getRevealedImprovementType(pUnit->getTeam()) == GD_INT_GET(BARBARIAN_CAMP_IMPROVEMENT) && !pPlot->isVisible(pUnit->getTeam()))
	{
		return false;
	}
