...
int CustomMods::getOption(const string& sOption, int defValue)
{
	if (!m_bInit)
	{
		...
		MOD_OPT_CACHE(CORE_NO_NAVAL_RANGED_ATTACKS_FROM_CITIES);
		...
		MOD_OPT_CACHE(COREUI_SHUFFLE_SPY_NAMES);
		MOD_OPT_CACHE(COREUI_COLLABORATOR_SPY_NAMES);
		...
		MOD_OPT_CACHE(BALANCE_UNIT_UPGRADE_COST);
		...
		MOD_OPT_CACHE(BALANCE_NO_COUPS);
		...
		MOD_OPT_CACHE(BALANCE_MINORS_CANNOT_ENTER_ENCAMPMENTS);
		...
		MOD_OPT_CACHE(BALANCE_CACHE_CITY_MEDIAN);
		...
		MOD_OPT_CACHE(UNITS_CIVILIANS_GAIN_XP);
