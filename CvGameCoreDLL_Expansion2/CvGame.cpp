			...
			for(int iJ = 0; iJ < MAX_TEAMS; iJ++)
			{
				...
				if(bGrantFreeTechs)
				{
					...
					if(kTeam.isAlive())
					{
						...
						if(!bValid)
						{
							if(((getHandicapInfo().isFreeTechs(iI)) != 0) ||
							        (!(kTeam.isHuman())&& (getHandicapInfo().isAIFreeTechs(iI) != 0)) ||
				...
				else
				{
					...
					else
					{
						...
						else
						{
							...
							{
								if(!gDLL->HasSentTurnComplete() && gDLL->allAICivsProcessedThisTurn() && allUnitAIProcessed() && (pkIface != 0) && pkIface->IsMPAutoEndTurnEnabled())
								{
									if (MOD_API_ACHIEVEMENTS)
										activePlayer.GetPlayerAchievements().EndTurn();
...
void CvGame::cycleCities(bool bForward, bool bAdd)
{
	...
	if((pkHeadSelectedCity != NULL) && ((pkHeadSelectedCity->getTeam() == getActiveTeam()) || isDebugMode()))
	{
		...
		while((pLoopCity != pkHeadSelectedCity) && (pSelectCity == 0));
...
void CvGame::cycleUnits(bool bClear, bool bForward, bool bWorkers)
{
	...
	if(pNextUnit != NULL /*&& bWrap */&& (pCycleCity == 0) && !bProcessed)
	{
		CvAssert(pNextUnit->getOwner() == eActivePlayer);
		selectUnit(pNextUnit, bClear);
	}
	...
	if((pCycleUnit != pCurrentSelectedUnit) || ((pCycleUnit != NULL) && pCycleUnit->ReadyToSelect()) || (pCycleCity != 0))
	{
		...
		if(pkSelectedUnit->getOwner() == getActivePlayer() && !pSelectedUnit->IsBusy())
		{
			...
			else if((eMessage == GAMEMESSAGE_PUSH_MISSION) || (eMessage == GAMEMESSAGE_AUTO_MISSION))
			{
				if(eMessage == GAMEMESSAGE_PUSH_MISSION)
				{
					...
					if((pPlot != 0) && pkSelectedUnit->CanSwapWithUnitHere(*pPlot) && eMission != CvTypes::getMISSION_ROUTE_TO())
					{
...
void CvGame::SelectSettler(void)
{
	...
	if((pSettlerUnit != 0) && pSettlerUnit->ReadyToSelect())
	{
			...
			if(iMissionType == CvTypes::getMISSION_BUILD())
			{
				...
				if(!pBuildInfo->isRepair())
				{
					...
					if(pBuildInfo->getRoute() == NO_ROUTE)
					{
						...
						if ((pImprovementInfo == 0) || !pImprovementInfo->IsRequiresImprovement())
						{
							if(pHeadSelectedUnit)
							{
								CvUnit* pkHeadSelectedUnit = GC.UnwrapUnitPointer(pHeadSelectedUnit.get());
...
bool CvGame::DoSpawnUnitsAroundTargetCity(PlayerTypes ePlayer, CvCity* pCity, int iNumber, bool bIncludeUUs, bool bIncludeShips, bool bNoResource, bool bIncludeOwnUUsOnly)
{
	...
	for(int iPlotLoop = 1; iPlotLoop < pCity->GetNumWorkablePlots(); iPlotLoop++)
	{
		...
		if(pPlot->defenseModifier(BARBARIAN_TEAM, false, false) != 0)
			iTempWeight += 4;
...
UnitTypes CvGame::GetRandomUniqueUnitType(bool bIncludeCivsInGame, bool bIncludeStartEra, bool bIncludeOldEras, bool bIncludeRanged, bool bCoastal, int iPlotX, int iPlotY)
{
	...
	for(int iMinorLoop = MAX_MAJOR_CIVS; iMinorLoop < MAX_CIV_PLAYERS; iMinorLoop++)
	{
		...
		if((pMinorLoop != 0) && pMinorLoop->isEverAlive())
		{
...
void CvGame::DoTestConquestVictory()
{
	...
	for (int iVictoryLoop = 0; iVictoryLoop < GC.getNumVictoryInfos(); iVictoryLoop++)
	{
		...
		if ((pkVictoryInfo != 0) && pkVictoryInfo->isConquest())
		{
...
int CalculateDigSiteWeight(int iIndex, vector<CvArchaeologyData>& inputData, vector<CvArchaeologyData>& chosenDigSites)
{
	...
	if (chosenDigSites[iIndex].m_eArtifactType == NO_GREAT_WORK_ARTIFACT_CLASS) // if we have not already chosen this spot for a dig site
	{
		...
		if ((iBaseWeight != 0) && pPlot->getFeatureType() != NO_FEATURE)
		{
		...
		if ((iBaseWeight != 0) && pPlot->getImprovementType() != NO_IMPROVEMENT)
		{
...
void CvGame::PopulateDigSite(CvPlot& kPlot, EraTypes eEra, GreatWorkArtifactClass eArtifact)
{
	...
	pNearestCity = pNearestCity != 0 ? pNearestCity : theMap.findCity(iPlotX, iPlotY, NO_PLAYER, NO_TEAM, false /* bSameArea */); // expand search if we need to
	if (pNearestCity != 0)
	{
	...
	else //  we can't find a nearby city (likely a late era start)
	{
		...
		else
		{
			...
			if (theMap.findNearestStartPlot(iPlotX, iPlotY, thisPlayer) != 0)
			{
