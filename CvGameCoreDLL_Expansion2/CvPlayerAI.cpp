...
GreatPeopleDirectiveTypes CvPlayerAI::GetDirectiveGeneral(CvUnit* pGreatGeneral)
{
	...
	{
		CvAIOperationBuildImprovementEscorted* pEscortOperation = OperationalAIHelpers::GetBuilderEscortOperation(GetID(), pGreatGeneral);

		// Units in escorts are either moving to build an improvement (e.g. citadel) or are just being escorted to a different city
		if (pEscortOperation && pEscortOperation->GetDirective().m_eBuild != NO_BUILD)
			return GREAT_PEOPLE_DIRECTIVE_CONSTRUCT_IMPROVEMENT;

		// Units in non-escort armies are field commanders
		if (!bHasGeneralNegation && !pEscortOperation)
			return GREAT_PEOPLE_DIRECTIVE_FIELD_COMMAND;
	}
...
GreatPeopleDirectiveTypes CvPlayerAI::GetDirectiveAdmiral(CvUnit* pGreatAdmiral)
{
	...
	{
		CvAIOperationBuildImprovementEscorted* pEscortOperation = OperationalAIHelpers::GetBuilderEscortOperation(GetID(), pGreatAdmiral);
		...
		// Units in non-escort armies are field commanders
		if (!bHasAdmiralNegation && !pEscortOperation)
			return GREAT_PEOPLE_DIRECTIVE_FIELD_COMMAND;
	}
