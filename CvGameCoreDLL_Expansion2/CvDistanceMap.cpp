...
{
	if (plot.GetPlotIndex() < m_vDistance.size())
...
{
	if (plot.GetPlotIndex() < m_vClosestFeature.size())
...
{
	if (iPlotIndex < m_vDistance.size())
	{
		//direct update
...
{
	if (eSpecificPlayer!=NO_PLAYER && eSpecificPlayer < majorPlayers.size())
