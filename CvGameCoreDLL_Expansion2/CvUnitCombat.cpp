...
#endif
		{
			int iCaptureRoll = GC.getGame().getRandNum(100, "Capture Enemy Roll");
			...
			if(pkDefender)
			{
				...
				if(pkAttacker)
				{
					...
					else
					{
						...
						{
							int iRand = GC.getGame().getRandNum(100, "Retreat Rand");
...
void CvUnitCombat::GenerateAirCombatInfo(CvUnit& kAttacker, CvUnit* pkDefender, CvPlot& plot, CvCombatInfo* pkCombatInfo)
{
	...
	if(pInterceptor != NULL && pInterceptor != pkDefender)
	{
		...
		if(GC.getGame().getRandNum(100, "Evasion Rand") >= kAttacker.evasionProbability())
		{
			// Is the interception successful?
			if(GC.getGame().getRandNum(100, "Intercept Rand (Air)") < pInterceptor->currInterceptionProbability())
			{
				...
				if(pLoopCity == NULL)
				{
					if(!pLoopPlot->isWater() && pLoopPlot->isValidMovePlot( pLoopPlot->getOwner() ))
					{
						if(pLoopPlot->getFeatureType() != NO_FEATURE)
						{
							...
							{
								if(pLoopPlot == pkTargetPlot || GC.getGame().getRandNum(100, "Nuke Fallout") < GC.getNUKE_FALLOUT_PROB())
								{
			...
			if(pkCity)
			{
				...
				else
				{
					...
					if(iDamageLevel == 1)
					{
						...
						iRandDamage1 = GC.getGame().getRandNum(/*20*/ GC.getNUKE_LEVEL1_POPULATION_DEATH_RAND_1(), "Population Nuked 1");
						iRandDamage2 = GC.getGame().getRandNum(/*20*/ GC.getNUKE_LEVEL1_POPULATION_DEATH_RAND_2(), "Population Nuked 2");
					}
					else
					{
						iBaseDamage = /*60*/ GC.getNUKE_LEVEL2_POPULATION_DEATH_BASE();
						iRandDamage1 = GC.getGame().getRandNum(/*10*/ GC.getNUKE_LEVEL2_POPULATION_DEATH_RAND_1(), "Population Nuked 1");
						iRandDamage2 = GC.getGame().getRandNum(/*10*/ GC.getNUKE_LEVEL2_POPULATION_DEATH_RAND_2(), "Population Nuked 2");
					}
					...
					if(pLoopUnit != NULL)
					{
						if(pLoopUnit != pkAttacker)
						{
							if(!pLoopUnit->isNukeImmune() && !pLoopUnit->isDelayedDeath())
							{
								...
								{
									iNukeDamage = (/*3*/ GC.getNUKE_UNIT_DAMAGE_BASE() + /*4*/ GC.getGame().getRandNum(GC.getNUKE_UNIT_DAMAGE_RAND_1(), "Nuke Damage 1") + /*4*/ GC.getGame().getRandNum(GC.getNUKE_UNIT_DAMAGE_RAND_2(), "Nuke Damage 2"));
								}
								// Wipe everything out
								else
								{
...
bool CvUnitCombat::ParadropIntercept(CvUnit& pParaUnit, CvPlot& pDropPlot) {
	...
	if (pInterceptor) {
		...
		// Is the interception successful?
		if(GC.getGame().getRandNum(100, "Intercept Rand (Paradrop)") < pInterceptor->currInterceptionProbability())
		{
