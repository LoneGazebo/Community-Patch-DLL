...
//	--------------------------------------------------------------------------------
#if defined(MOD_BALANCE_CORE_JFD)
void CvUnit::init(int iID, UnitTypes eUnit, UnitAITypes eUnitAI, PlayerTypes eOwner, int iX, int iY, DirectionTypes eFacingDirection, bool bNoMove, bool bSetupGraphical, int iMapLayer /*= DEFAULT_UNIT_MAP_LAYER*/, int iNumGoodyHutsPopped, ContractTypes eContract)
#else
...
{
#if defined(MOD_BALANCE_CORE_JFD)
	initWithNameOffset(iID, eUnit, -1, eUnitAI, eOwner, iX, iY, eFacingDirection, bNoMove, bSetupGraphical, iMapLayer, iNumGoodyHutsPopped, eContract);
#else
...
void CvUnit::initWithSpecificName(int iID, UnitTypes eUnit, const char* strKey, UnitAITypes eUnitAI, PlayerTypes eOwner, int iX, int iY, DirectionTypes eFacingDirection, bool bNoMove, bool bSetupGraphical, int iMapLayer, int iNumGoodyHutsPopped)
{
	...
	else if (MOD_GLOBAL_RELIGIOUS_SETTLERS && (getUnitInfo().IsFound() || getUnitInfo().IsFoundAbroad() || getUnitInfo().IsFoundMid() || getUnitInfo().IsFoundLate()))
	{
		...
		if (pPlotCity)
		{
			...
			int randFollower = GC.getGame().getRandNum(totalFollowers, "Religious Settlers: Picking a random religion for the settler") + 1;
...
//	--------------------------------------------------------------------------------
#if defined(MOD_BALANCE_CORE)
void CvUnit::initWithNameOffset(int iID, UnitTypes eUnit, int iNameOffset, UnitAITypes eUnitAI, PlayerTypes eOwner, int iX, int iY, DirectionTypes eFacingDirection, bool bNoMove, bool bSetupGraphical, int iMapLayer, int iNumGoodyHutsPopped, ContractTypes eContract)
#else
...
#endif
{
...
#endif
	{
		if(iNameOffset == -1)
		{
			iNameOffset = GC.getGame().getRandNum(iNumNames, "Unit name selection");
		}
#if defined(MOD_BALANCE_CORE)
		...
		{
			int iRoll = GC.getGame().getRandNum(vfPossibleUnits.size(), "Rolling for era-specific unit");
...
int CvUnit::getCombatDamage(int iStrength, int iOpponentStrength, int iCurrentDamage, bool bIncludeRand, bool bAttackerIsCity, bool bDefenderIsCity) const
{
	...
	if(bIncludeRand)
	{
		iRoll = /*400*/ GC.getGame().getRandNum(GC.getATTACK_SAME_STRENGTH_POSSIBLE_EXTRA_DAMAGE(), "Unit Combat Damage");
		iRoll *= iDamageRatio;
#if defined(MOD_UNITS_MAX_HP)
						...
						if((getDomainType() != DOMAIN_AIR) || pLoopPlot->isFriendlyCity(*this, true))
						{
							if(getDomainType() != DOMAIN_SEA || (pLoopPlot->isFriendlyCity(*this, true) && pLoopPlot->isCoastalLand()) || (pLoopPlot->isWater() && !pLoopPlot->isLake()))
							{
								if(pLoopPlot->isRevealed(getTeam()))
								{
									...
									if(iValue < iBestValue || (iValue == iBestValue && GC.getGame().getRandNum(3,"jump to plot")<2) )
									{
										iBestValue = iValue;
										pBestPlot = pLoopPlot;
									}
...
void CvUnit::DoAttrition()
{
	...
	if(!IsInFriendlyTerritory())
	{
		...
		{
			if(GC.getGame().getRandNum(100, "Enemy Territory Damage Chance") < getEnemyDamageChance())
			{
		...
		{
			if(GC.getGame().getRandNum(100, "Enemy Territory Heal Chance") <= getEnemyDamageChance())
			{
		...
		{
			if(GC.getGame().getRandNum(100, "Neutral Territory Damage Chance") < getNeutralDamageChance())
			{
				strAppendText =  GetLocalizedText("TXT_KEY_MISC_YOU_UNIT_WAS_DAMAGED_ATTRITION");
...
#endif
				iPillageGold = GC.getGame().getRandNum(pkImprovement->GetPillageGold(), "Pillage Gold 1");
				iPillageGold += GC.getGame().getRandNum(pkImprovement->GetPillageGold(), "Pillage Gold 2");
...
int CvUnit::GetAirCombatDamage(const CvUnit* pDefender, CvCity* pCity, bool bIncludeRand, int iAssumeExtraDamage, const CvPlot* pTargetPlot, const CvPlot* pFromPlot) const
{
	...
	if(bIncludeRand)
	{
		iAttackerRoll = /*300*/ GC.getGame().getRandNum(GC.getRANGE_ATTACK_SAME_STRENGTH_POSSIBLE_EXTRA_DAMAGE(), "Unit Ranged Combat Damage");
...
int CvUnit::GetAirStrikeDefenseDamage(const CvUnit* pAttacker, bool bIncludeRand, const CvPlot* pTargetPlot, const CvPlot* pFromPlot) const
{
	...
	if (bIncludeRand)
		return 8 + GC.getGame().getRandNum(5,"air attack attrition");
	else
		return 10;
#else
	if (!pAttacker)
...
int CvUnit::GetInterceptionDamage(const CvUnit* pAttacker, bool bIncludeRand, const CvPlot* pTargetPlot, const CvPlot* pFromPlot) const
{
	...
	if(bIncludeRand)
	{
		iInterceptorRoll = /*300*/ GC.getGame().getRandNum(GC.getINTERCEPTION_SAME_STRENGTH_POSSIBLE_EXTRA_DAMAGE(), "Interception Combat Damage");
...
void CvUnit::DoNearbyUnitPromotion(CvPlot* pPlot)
{
	if (pPlot == NULL)
	{
		pPlot = plot();
	...
	if(pPlot != NULL)
	{
		...
		for(iI = 0; iI < GC.getNumPromotionInfos(); iI++)
		{
			...
			if(pkPromotionInfo != NULL)
			{
				...
				{
					if(!IsWithinDistanceOfUnitPromotion(pkPromotionInfo->AddedFromNearbyPromotion(), pkPromotionInfo->NearbyRange(), true, false))
					{
						setHasPromotion(eLoopPromotion, false);
					}
					else
...
#if defined(MOD_GLOBAL_STACKING_RULES)
	kStream >> m_stackingUnit.eOwner;
	kStream >> m_stackingUnit.iID;
#endif
...
bool CvUnit::CanWithdrawFromMelee(CvUnit& attacker)
{
	...
	int iRoll = GC.getGame().getRandNum(100, "Withdraw from Melee attempt");
...
bool CvUnit::DoWithdrawFromMelee(CvUnit& attacker)
{
	...
	int iRightOrLeftBias = (GC.getGame().getRandNum(100, "right or left bias") < 50) ? 1 : -1;
...
void CvUnit::DoPlagueTransfer(CvUnit& defender)
{
	...
	int iRoll = GC.getGame().getRandNum(100, "Plague Transfer from Melee attempt");
...
int CvUnit::AI_promotionValue(PromotionTypes ePromotion)
{
	...
	if(iValue > 0)
	{
		iValue += GC.getGame().getRandNum(15, "AI Promote");
	}
...
const CvUnit* CvUnit::getStackingUnit() const
{
	VALIDATE_OBJECT
	return getUnit(m_stackingUnit);
}

//	--------------------------------------------------------------------------------
CvUnit* CvUnit::getStackingUnit()
{
	VALIDATE_OBJECT
	return getUnit(m_stackingUnit);
}
void CvUnit::setStackingUnit(CvUnit* pStackingUnit)
{
	VALIDATE_OBJECT
	if(pStackingUnit != NULL)
	{
		if(pStackingUnit->IsStackingUnit())
		{
			m_stackingUnit = pStackingUnit->GetIDInfo();
		}
	}
	else
	{
...
void CvUnit::DoCargoPromotions(CvUnit& cargounit)
{
	if(cargounit.IsCargoCombatUnit() && cargounit.hasCargo())
	{
		...
		if(isCargo())
		{
			...
			{
				setMoves((getUnitInfo().GetMoves() * 60) + 120);
			}
		}
	}
}
