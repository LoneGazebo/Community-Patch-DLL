...
int CvUnit::healRate(const CvPlot* pPlot) const
{
	...
	// - Religion for MOD_RELIGION_PERMANENT_PANTHEON is state religion
	...
	// - Religion for MOD_RELIGION_PERMANENT_PANTHEON is the nearby city's majority religion
...
void CvUnit::promote(PromotionTypes ePromotion, int iLeaderUnitId)
{
	...
	else
	{
		...
		if (MOD_BALANCE_VP && !IsCannotHeal())
		{
...
std::map<CvUnit*, CvPlot*> CvUnit::DoSquadPlotAssignments(CvPlot* pDestPlot, bool escort, bool computeOnly)
{
	...
	for (int targetPlotIdx = 0; targetPlotIdx < RING_PLOTS[currRingEndIdx]; targetPlotIdx++)
	{
		...
		if (targetPlotIdx == (RING_PLOTS[currRingEndIdx] - 1) && eligiblePlots.size() < unique_plots_required)
		{
			currRingEndIdx++;
		}
	}
