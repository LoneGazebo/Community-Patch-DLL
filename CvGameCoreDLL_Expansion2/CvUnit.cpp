...
bool CvUnit::CanCultureBomb(const CvPlot* pPlot, bool bTestVisible) const
{
	...
	if(!bTestVisible)
	{
		...
		if(pPlot != NULL)
		{
			// Can't be inside someone else's territory (unless we have the special ability)
			if(pPlot->getOwner() != NO_PLAYER && pPlot->getOwner() != getOwner() && !GET_PLAYER(getOwner()).IsCultureBombAnywhere())
				return false;
