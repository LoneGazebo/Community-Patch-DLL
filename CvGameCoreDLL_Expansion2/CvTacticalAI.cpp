...
void CvTacticalAI::PlotCaptureCityMoves()
{
	...
	while(pTarget != NULL)
	{
		...
		if(pPlot != NULL && pPlot->isCity())
		{
			...
			if(FindUnitsWithinStrikingDistance(pPlot, false, false))
			{
				...
				if ( (iExpectedDamagePerTurn - GC.getCITY_HIT_POINTS_HEALED_PER_TURN())*iMaxSiegeTurns < iRequiredDamage )
				{
					if (GC.getLogging() && GC.getAILogging() && pZone)
					{
						...
						strLogString.Format("Zone %d, too early for attacking %s, required damage %d, expected damage per turn %d", 
							pZone ? pZone->GetZoneID() : -1, pCity->getName().c_str(), iRequiredDamage, iExpectedDamagePerTurn);
						LogTacticalMessage(strLogString);
					}
					pTarget = GetNextZoneTarget();
					continue;
...
void CvTacticalAI::PlotPillageMoves(AITacticalTargetType eTarget, bool bImmediate)
{
	...
	for(CvTacticalTarget* pTarget = GetFirstZoneTarget(eTarget); pTarget != NULL; pTarget = GetNextZoneTarget())
	{
		...
		if (!pPlot->isWater())
		{
			...
			else if (bImmediate && FindUnitsCloseToPlot(pPlot, 0, GC.getMAX_HIT_POINTS() / 5, GC.getMAX_HIT_POINTS() - iMinDamage, DOMAIN_LAND, true, true))
			{
				// Queue best one up to capture it
			...
			else if (!bImmediate && FindUnitsCloseToPlot(pPlot, 2, GC.getMAX_HIT_POINTS() / 5, GC.getMAX_HIT_POINTS() - iMinDamage, DOMAIN_LAND, true, true))
			{
		...
		{
			if (bImmediate && FindUnitsCloseToPlot(pPlot, 0, GC.getMAX_HIT_POINTS() / 5, GC.getMAX_HIT_POINTS() - iMinDamage, DOMAIN_SEA, true, true))
			{
				// Queue best one up to capture it
				if (ExecutePillage(pPlot))
				{
					...
					{
						CvString strLogString;
						strLogString.Format("SEA Pillaging %s, X: %d, Y: %d", szTargetName.GetCString(), pTarget->GetTargetX(), pTarget->GetTargetY());
						LogTacticalMessage(strLogString);
					}
				}
			}
			else if (!bImmediate && FindUnitsCloseToPlot(pPlot, 2, GC.getMAX_HIT_POINTS() / 5, GC.getMAX_HIT_POINTS() - iMinDamage, DOMAIN_SEA, true, true))
			{
				//be careful when sending out single units ...
				CvTacticalDominanceZone* pZone = GetTacticalAnalysisMap()->GetZoneByPlot(pPlot);
				if (!pZone || pZone->GetOverallDominanceFlag() == TACTICAL_DOMINANCE_ENEMY)
				...
				if (pUnit && pUnit->canMoveInto(*pPlot, CvUnit::MOVEFLAG_DESTINATION))
				{
					...
					{
						CvString strLogString;
						strLogString.Format("Moving toward %s for SEA pillage, X: %d, Y: %d", szTargetName.GetCString(), pTarget->GetTargetX(), pTarget->GetTargetY());
						LogTacticalMessage(strLogString);
					}
...
void CvTacticalAI::PlotBlockadeMoves()
{
	...
	while(pTarget != NULL)
	{
		// See what units we have who can reach target this turn
		CvPlot* pPlot = GC.getMap().plot(pTarget->GetTargetX(), pTarget->GetTargetY());
		if (FindUnitsCloseToPlot(pPlot, 4, 10, 100, DOMAIN_SEA, pPlot->getImprovementType() != NO_IMPROVEMENT, true))
		{
			// Queue best one up to capture it
...
void CvTacticalAI::ExecuteFormationMoves(CvArmyAI* pArmy, CvPlot *pTurnTarget)
{
	...
	if(!ScoreDeploymentPlots(pTurnTarget, pArmy, iMeleeUnits, iRangedUnits, iRange))
	{
		...
		{
			CvString strLogString;
			strLogString.Format("Operation %d aborting. Army ID: %d. Not enough spaces to deploy along formation's path", pArmy->GetOperationID(), pArmy->GetID());
			LogTacticalMessage(strLogString);
		}
		m_pPlayer->getAIOperation(pArmy->GetOperationID())->SetToAbort(AI_ABORT_NO_ROOM_DEPLOY);
		return;
		...
		for(unsigned int iI = 0; iI < m_TempTargets.size() && !bDone; iI++)
		{
			...
			else
			{
				...
				if(FindClosestOperationUnit( pCurrent, unitMovePlots, false /*bIncludeRanged*/, false /*bMustBeRangedUnit*/, false))
				{
					...
					{
						CvString strMsg;
						strMsg.Format("Op %d, Deploying melee unit (first pass) %d (%s), to %d,%d, from %d,%d", pArmy->GetOperationID(), pInnerUnit->GetID(), pInnerUnit->getName().GetCString(), pLoopPlot->getX(), pLoopPlot->getY(), pInnerUnit->getX(), pInnerUnit->getY());
						LogTacticalMessage(strMsg);
					}
			...
			if (eTargetType == AI_TACTICAL_TARGET_HIGH_PRIORITY_UNIT)
			{
				...
				else
				{
					...
					if(FindClosestOperationUnit( pCurrent, unitMovePlots, true /*bIncludeRanged*/, true /*bMustBeRangedUnit*/, false))
					{
						...
						{
							CvString strMsg;
							strMsg.Format("Op %d, Deploying ranged unit (first pass) %d (%s), to %d,%d, from %d,%d", pArmy->GetOperationID(), pInnerUnit->GetID(), pInnerUnit->getName().GetCString(), pLoopPlot->getX(), pLoopPlot->getY(), pInnerUnit->getX(), pInnerUnit->getY());
							LogTacticalMessage(strMsg);
						}
						ExecuteMoveToPlot(pInnerUnit, pLoopPlot);
						iRangedUnitsToPlace--;
	...
	for(unsigned int iI = 0; iI < m_TempTargets.size() && !bDone; iI++)
	{
		...
		else
		{
			...
			if(FindClosestOperationUnit( pCurrent, unitMovePlots, bSuitableForRanged /*bIncludeRanged*/, false /*bMustBeRangedUnit*/, false))
			{
				...
				{
					CvString strMsg;
					strMsg.Format("Op %d, Deploying third wave unit %d (%s), second pass, to %d,%d, from %d,%d", pArmy->GetOperationID(), pInnerUnit->GetID(), pInnerUnit->getName().GetCString(), pLoopPlot->getX(), pLoopPlot->getY(), pInnerUnit->getX(), pInnerUnit->getY());
					LogTacticalMessage(strMsg);
				}
...
bool CvTacticalAI::ExecutePillage(CvPlot* pTargetPlot)
{
	// Move first one to target
	CvUnit* pUnit = m_pPlayer->getUnit(m_CurrentMoveUnits[0].GetID());
	bool bShouldPillage = pUnit->shouldPillage(pTargetPlot);
	if (pUnit && bShouldPillage && pUnit->canMoveInto(*pTargetPlot, CvUnit::MOVEFLAG_DESTINATION))
	{
		pUnit->PushMission(CvTypes::getMISSION_MOVE_TO(), pTargetPlot->getX(), pTargetPlot->getY());

		//now that the neighbor plots are revealed, maybe it's better to retreat?
		CvPlot* pSafePlot = NULL;
		if (!IsEnemyCitadel(pUnit->plot(), m_pPlayer->getTeam()) && !bShouldPillage)
		{
			...
			if (pBestPlot != NULL)
			{
				...
				else
				{
					...
					if (pBestPlot != NULL)
					{
						...
						if (pBestPlot->isAdjacent(pUnit->plot()) && pUnit->shouldPillage(pUnit->plot(), true))
						{
							pUnit->PushMission(CvTypes::getMISSION_PILLAGE());
							if (GC.getLogging() && GC.getAILogging())
							{
							...
							DomainTypes eDomain, bool bMustPillage, bool bAllowMoveThroughEnemyLand)
{
	...
	for(list<int>::iterator it = m_CurrentTurnUnits.begin(); it != m_CurrentTurnUnits.end(); it++)
	{
		...
		if(pLoopUnit && !pLoopUnit->TurnProcessed() && pLoopUnit->IsCombatUnit()) //ignore generals and the like!
		{
			if (bMustPillage)
			{
				if (!pLoopUnit->shouldPillage(pTarget))
					continue;
			}
			...
			int iHPDelta = pLoopUnit->GetMaxHitPoints() - GC.getMAX_HIT_POINTS();

			if (iMaxHitpoints>0 && pLoopUnit->GetCurrHitPoints()>(iMaxHitpoints + iHPDelta))
				continue;
				...
				bool bIgnoreUnitAdjacencyBoni, int iExtraDefenderDamage, bool bQuickAndDirty)
{
	...
	if (pAttacker->IsCanAttackRanged())
	{
		...
		{
			iDamage += pAttacker->GetRangeCombatDamage(pDefender, NULL, false, 0, 
	...
	else
	{
		...
		iAttackerDamage = pDefender->getCombatDamage(
			iDefenderStrength,
			iAttackerStrength,
			pDefender->getDamage() + iExtraDefenderDamage, 
			false, false, false);
	}
	...
	else //empty plot or friendly unit. the latter case will be handled by the isMoveBlockedByOtherUnit check
	{
		...
		for (int iRange = 1; iRange < iMaxRange + 1; iRange++)
		{
			...
			if (iRange == 1 && currentPlot.getType() != CvTacticalPlot::TP_FRONTLINE)
				continue;
			if (iRange == 2 && currentPlot.getType() != CvTacticalPlot::TP_FRONTLINE && currentPlot.getType() != CvTacticalPlot::TP_SECONDLINE)
				continue;
		...
		if (currentPlot.isEnemyCivilian()) //unescorted civilian
		{
			if (plot.iMovesLeft > 0 || currentPlot.getNumAdjacentEnemies() == 0)
			{
				...
				if (pCivilian)
				{
					...
					result.eType = STacticalAssignment::A_CAPTURE; //important so that the next assigment can be a move again
				}
			}
		}
		...
		if (bEndTurn) //can happen if we run out of movement points or if we actively decide to stay in a plot
		{
			if (plot.iPlotIndex == unit.iPlotIndex)
			{
				...
				if (pUnit->shouldPillage(pCurrentPlot, false, plot.iMovesLeft) && !assumedPosition.unitHasAssignmentOfType(unit.iUnitID, STacticalAssignment::A_PILLAGE))
				{
					...
					else if (pCurrentPlot->getImprovementType() != NO_IMPROVEMENT && (plot.iMovesLeft > 0 || pUnit->hasFreePillageMove()))
					{
						iMiscScore += 20;
						if (pUnit->IsGainsXPFromPillaging())
							iMiscScore += 10;
				...
				if (result.eType != STacticalAssignment::A_PILLAGE)
				{
					...
					{
						if (pUnit->IsHurt())
							iMiscScore += 3; //cannot fortify, only heal
			...
			iMiscScore += iPlotTypeScores[unit.eStrategy][currentPlot.getType()];
		...
		result.iScore = iDamageScore * 10 + iMiscScore * 10 - iDangerScore + 20;
...
STacticalAssignment ScorePlotForCombatUnitDefensive(const SUnitStats unit, SMovePlot plot, const CvTacticalPosition& assumedPosition)
{
	...
	{
		if (pUnit->IsHurt())
			result.iScore += 3; //for healing
		if (!pUnit->noDefensiveBonus())
			result.iScore += 3; //fortification bonus!
	}
...
vector<STacticalAssignment> CvTacticalPosition::getPreferredAssignmentsForUnit(SUnitStats unit, int nMaxCount) const
{
	...
	bool bCantMoveAgainRightNow = (pUnit->isRanged() && unit.eLastAssignment == STacticalAssignment::A_MOVE);
	if (!bCantMoveAgainRightNow)
	{
		...
		{
			//some quick checks
			const CvTacticalPlot& currentPlot = getTactPlot(it->iPlotIndex);
			if (!currentPlot.isValid() || !currentPlot.isRelevant())
				continue;
			//prevent two moves in a row, that is inefficient and can lead to "shuttling" behavior
			if (unit.eLastAssignment == STacticalAssignment::A_MOVE && !currentPlot.isEnemy())
				continue;
	...
	if (pUnit->isRanged() && eAggression>AL_NONE && unit.iAttacksLeft>0 && unit.iMovesLeft>0)
	{
		...
		for (set<int>::const_iterator it=rangeAttackPlots.begin(); it!=rangeAttackPlots.end(); ++it)
		{
			...
			if (enemyPlot.isValid() && enemyPlot.isEnemy() && assumedUnitPlot.isValid()) //still alive?
			{
				...
				unitAfterAttack.eLastAssignment = move.eType; //doesn't really matter as long as it's not A_MOVE
				...
				if (enemyPlot.getNumAdjacentFirstlineFriendlies()>0)
					move.iScore += 2; //a slight boost for attacking the enemies frontline
...
bool CvTacticalPosition::makeNextAssignments(int iMaxBranches, int iMaxChoicesPerUnit, CvTactPosStorage& storage)
{
	...
	*/
	...
	for (vector<STacticalAssignment>::iterator itMove = overAllChoices.begin(); itMove != overAllChoices.end(); ++itMove)
	{
		...
		if (itMove->eType==STacticalAssignment::A_BLOCKED && overAllChoices.size()>1)
			continue;
...
void CvTacticalPosition::updateMoveAndAttackPlotsForUnit(SUnitStats unit)
{
	...
	for (ReachablePlots::iterator it = reachablePlots.begin(); it != reachablePlots.end(); ++it)
	{
		...
		if (!pUnit->isNativeDomain(pPlot) && !pPlot->isEnemyCity(*pUnit)) //allow melee ships to capture cities though!
			continue;
...
bool CvTacticalPosition::addAvailableUnit(const CvUnit* pUnit)
{
	...
	if (pUnit->isEmbarked()) //embarked units try and stay away from enemies
