...
void CvTacticalAI::PlotDestroyUnitMoves(AITacticalTargetType targetType, bool bMustBeAbleToKill, bool bAttackAtPoorOdds)
{
	...
	for (size_t i=0; i<targets.size(); i++)
	{
		...
		for(unsigned int iI = 0; iI < m_CurrentMoveCities.size(); iI++)
		{
			...
			{
				CvString strMsg;
				CvString strTemp = pCity->getNameNoSpace();
...
void CvTacticalAI::PlotArmyMovesCombat(CvArmyAI* pThisArmy)
{
	...
	{
		bool bShowOfForce = (m_pPlayer->GetDiplomacyAI()->GetWarGoal(pOperation->GetEnemy()) == WAR_GOAL_DEMAND) || (pOperation->GetOperationType() == AI_OPERATION_BULLY_CITY_STATE);
	...
	for (int i = 0; i < 6; i++)
	{
		...
		if (pNeighbor)
		{
			...
			if (tactPlot.isValid())
			{
				...
				if (tactPlot.hasFriendlyCombatUnit() || tactPlot.getType()==TP_BLOCKED_FRIENDLY)
				...
				if (tactPlot.getType() == TP_FRONTLINE && (tactPlot.hasFriendlyCombatUnit() || tactPlot.getType()==TP_BLOCKED_FRIENDLY))
