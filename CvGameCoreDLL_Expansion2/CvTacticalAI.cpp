	...
	for(int iI = 0; iI < m_pMap->GetNumZones(); iI++)
	{
		...
		if(UseThisDominanceZone(pZone))
		{
			...
			if(GC.getLogging() && GC.getAILogging() && eNewPosture != AI_TACTICAL_POSTURE_NONE)
			{
				...
				if(pZone->GetClosestCity() != NULL)
				{
					szPostureMsg += pZone->GetClosestCity()->getName();
				}
		...
		if(bValidPlot)
		{
			...
			else
			{
				...
				if(pUnit != NULL)
				{
					...
					newTarget.SetAuxIntData(50+pUnit->getDamage());
					m_AllTargets.push_back(newTarget);
				}
...
#if defined(MOD_BALANCE_CORE_MILITARY)
					if(FindUnitsWithinStrikingDistance2(pPlot, 1, bNoRangedUnits, false /*bNavalOnly*/, false /*bMustMoveThrough*/, true /*bIncludeBlockedUnits*/))
#else
...
void CvTacticalAI::PlotBarbarianCampMoves()
{
	...
	while(pTarget != NULL)
	{
		...
		CvPlot* pPlot = GC.getMap().plot(pTarget->GetTargetX(), pTarget->GetTargetY());
#if defined(MOD_BALANCE_CORE_MILITARY)
		if(FindClosestUnit(pPlot,2,false))
#else
...
void CvTacticalAI::PlotPlunderTradePlotMoves (DomainTypes eDomain)
{
	...
	while(pTarget != NULL)
	{
		// See what units we have who can reach target this turn
		CvPlot* pTargetPlot = GC.getMap().plot(pTarget->GetTargetX(), pTarget->GetTargetY());
		if (FindUnitsWithinStrikingDistance2(pTargetPlot, 30, false /* bNoRangedUnits */, bNavalOnly, false /*bMustMoveThrough*/, false /*bIncludeBlockedUnits*/))
		{
...
void CvTacticalAI::PlotEnemyTerritoryOperationMoves(CvAIOperationEnemyTerritory* pOperation)
{
	...
	else if(pThisArmy->GetArmyAIState() == ARMYAISTATE_MOVING_TO_DESTINATION)
	{
		...
		CvPlot* pThisTurnTarget = pOperation->ComputeCenterOfMassForTurn(pThisArmy, &pClosestCurrentCOMonPath);
...
void CvTacticalAI::PlotFreeformNavalOperationMoves(CvAIOperationNaval* pOperation)
{
	...
	else if(pThisArmy->GetArmyAIState() == ARMYAISTATE_MOVING_TO_DESTINATION)
	{
		...
		// Update army's current location
		CvPlot* pClosestCurrentCOMonPath = NULL;
		CvPlot* pCurrentPosition = pOperation->ComputeCenterOfMassForTurn(pThisArmy,&pClosestCurrentCOMonPath);
...
void CvTacticalAI::ExecuteGatherMoves(CvArmyAI* pArmy)
{
	...
	for(unsigned int iI = 0; iI < m_TempTargets.size() && !bDone; iI++)
	{
		...
		{
#if defined(MOD_BALANCE_CORE_MILITARY)
			if (FindClosestOperationUnit(pLoopPlot, true /*bIncludeRanged*/, false /*bMustBeRangedUnit*/, 5, 10, 4))
#else
	...
	else
	{
		...
		for(unsigned int iI = 0; iI < m_TempTargets.size() && !bDone; iI++)
		{
			...
			if (!pLoopPlot->getBestDefender(NO_PLAYER))
			{
				...
				if(FindClosestOperationUnit( GC.getMap().plot(pLoopPlot->getX()-iDX,pLoopPlot->getY()-iDY), false /*bIncludeRanged*/, false /*bMustBeRangedUnit*/, 2, 10, 1))
				{
				...
				else if(FindClosestOperationUnit( GC.getMap().plot(pLoopPlot->getX()-iDX,pLoopPlot->getY()-iDY), false /*bIncludeRanged*/, false /*bMustBeRangedUnit*/, 3, 10, 1))
				{
					UnitHandle pInnerUnit = m_pPlayer->getUnit(m_CurrentMoveUnits[0].GetID());
					if(GC.getLogging() && GC.getAILogging())
					{
		...
		for(unsigned int iI = 0; iI < m_TempTargets.size() && !bDone; iI++)
		{
			...
			if (eTargetType == AI_TACTICAL_TARGET_HIGH_PRIORITY_UNIT)
			{
				...
				if (!pLoopPlot->getBestDefender(NO_PLAYER))
				{
#if defined(MOD_BALANCE_CORE)
					//find a unit that's a little bit further back, and then try to move it to this good plot
					if(FindClosestOperationUnit( GC.getMap().plot(pLoopPlot->getX()-iDX,pLoopPlot->getY()-iDY), true /*bIncludeRanged*/, true /*bMustBeRangedUnit*/, 2, 10, 1))
					{
						UnitHandle pInnerUnit = m_pPlayer->getUnit(m_CurrentMoveUnits[0].GetID());
						if(GC.getLogging() && GC.getAILogging())
						{
					...
					else if(FindClosestOperationUnit( GC.getMap().plot(pLoopPlot->getX()-iDX,pLoopPlot->getY()-iDY), true /*bIncludeRanged*/, true /*bMustBeRangedUnit*/, 3, 10, 1))
					{
						UnitHandle pInnerUnit = m_pPlayer->getUnit(m_CurrentMoveUnits[0].GetID());
						if(GC.getLogging() && GC.getAILogging())
						{
...
#if defined(MOD_BALANCE_CORE)
				//find a unit that's a little bit further back, and then try to move it to this good plot
				if(FindClosestOperationUnit( GC.getMap().plot(pLoopPlot->getX()-iDX,pLoopPlot->getY()-iDY), true /*bIncludeRanged*/, false /*bMustBeRangedUnit*/, 3, 10, 1))
				{
					UnitHandle pInnerUnit = m_pPlayer->getUnit(m_CurrentMoveUnits[0].GetID());
					if(GC.getLogging() && GC.getAILogging())
					{
...
void CvTacticalAI::ExecutePillage(CvPlot* pTargetPlot)
{
	...
	if(pUnit)
	{
		if(pUnit->canMoveInto(*pTargetPlot, CvUnit::MOVEFLAG_DESTINATION | CvUnit::MOVEFLAG_PRETEND_CORRECT_EMBARK_STATE))
		{
			pUnit->PushMission(CvTypes::getMISSION_MOVE_TO(), pTargetPlot->getX(), pTargetPlot->getY());
			pUnit->PushMission(CvTypes::getMISSION_PILLAGE());
		}
...
void CvTacticalAI::ExecuteAttack(CvTacticalTarget* pTarget, CvPlot* pTargetPlot, bool bInflictWhatWeTake, bool bMustSurviveAttack)
{
...
#if defined(MOD_BALANCE_CORE_MILITARY)
	// Make a list of suitable adjacent plots
	...
	for(int iDirectionLoop = 0; iDirectionLoop < NUM_DIRECTION_TYPES; ++iDirectionLoop)
	{
		...
		if (pAdjacentPlot)
		{
			if (pAdjacentPlot->isImpassable(m_pPlayer->getTeam()) || pAdjacentPlot->isMountain())
				continue;

			//can't move into a plot with a foreign unit
			if(pAdjacentPlot->getNumDefenders(NO_PLAYER)>pAdjacentPlot->getNumDefenders(m_pPlayer->GetID()))
				continue;
			
			//can't move into a plot with a foreign city
			if(pAdjacentPlot->isCity() && pAdjacentPlot->getPlotCity()->getOwner()!=m_pPlayer->GetID())
				continue;
	...
	for(unsigned int iI = 0; iI < m_CurrentMoveUnits.size(); iI++)
	{
		...
		if (plotDistance(pUnit->getX(), pUnit->getY(), pTargetPlot->getX(), pTargetPlot->getY()) > 1)
		{
			...
			{
				//firstly, if there's another unit there (must be friendly), check if we can swap places
				...
				if (!pFriendlyUnit)
				{
					...
					if (TurnsToReachTarget(pUnit, *it, false /*bReusePaths*/, false /*bIgnoreUnits*/, false /*bIgnoreStacking*/, 1) > 0)
						continue;
					...
					if(!pUnit->IsCanAttackRanged())
					{
						...
						else
						{
							...
							for (it = plotList.begin(); it != plotList.end(); it++)
							{
								...
								if (TurnsToReachTarget(pUnit, *it, false /*bReusePaths*/, false /*bIgnoreUnits*/, false /*bIgnoreStacking*/, 1) > 2)
									continue;
							...
							if (!bQueueAttackNow)
							{
								...
								if (MoveToEmptySpaceNearTarget(pUnit, pTargetPlot, (pUnit->getDomainType() == DOMAIN_LAND)))
								{
									...
									{
										CvString strMsg;
										strMsg.Format("Moving adjacent for attack next turn (loop 2), Now at X: %d, Y: %d", pUnit->getX(), pUnit->getY());
										LogTacticalMessage(strMsg);
									}
								}
							}
					...
					if(pUnit->plot()->getOwner() != pUnit->getOwner())
					{
						if(pUnit->getDomainType() == DOMAIN_LAND)
						{
							...
							for (uint uiPlotIndex = 0; uiPlotIndex < aiPlots.size(); uiPlotIndex++)
							{
								...
								if(pLoopPlot != NULL && pLoopPlot->getNumUnits() <= 0 && !pLoopPlot->isWater() && !pLoopPlot->isImpassable(m_pPlayer->getTeam()))
								{
		...
		if(pUnit)
		{
			...
			if (pUnit->getDomainType()==DOMAIN_LAND)
			{
				...
				else if (pUnit->isEmbarked())
				{
					...
					pBetterPlot = TacticalAIHelpers::FindClosestSafePlotForHealing(pUnit.pointer(),false);
					if (!pBetterPlot)
						pBetterPlot = TacticalAIHelpers::FindSafestPlotInReach(pUnit.pointer(),false);
			...
			{
				if (m_pPlayer->GetPlotDanger(*pUnit->plot(),pUnit.pointer())>0 || !pUnit->canHeal(pUnit->plot()))
				{
					//find a friendly sea tile
					pBetterPlot = TacticalAIHelpers::FindClosestSafePlotForHealing(pUnit.pointer(),true);
					if (!pBetterPlot)
						pBetterPlot = TacticalAIHelpers::FindSafestPlotInReach(pUnit.pointer(),false);
					if (!pBetterPlot || pBetterPlot==pUnit->plot())
					{
			...
			for(unsigned int iI = 0; iI < m_CurrentMoveCities.size() && iRequiredDamage > 0; iI++)
			{
				...
				if(pCity != NULL)
				{
					if(QueueAttack(pCity, &kTarget, true /*bRanged*/, true /*bCity*/))
						LaunchAttack(pCity, &kTarget, true /*bFirst*/, true /*bFirstAttackRanged*/, true /*bFirstAttackCity*/);
				}
			}
		...
		for (TacticalAIHelpers::ReachablePlotSet::iterator it=movePlots.begin(); it!=movePlots.end(); ++it)
		{
			...
			if (candidates.find(it->first) != candidates.end())
			{
				...
				{ 
					...
					if (!pUnit->canMoveInto(*pBasePlot,CvUnit::MOVEFLAG_DESTINATION | CvUnit::MOVEFLAG_PRETEND_CORRECT_EMBARK_STATE))
						continue;
				}
					...
					if(iDistance > 0 && iDistance <= iRange)
					{
						...
						if(pCell->IsRevealed() && pCell->CanUseForOperationGathering())
						{
							...
							if(!pCell->IsSubjectToAttack())
							{
								...
								if(FindClosestUnit(pLoopPlot, iTurnsToReach, false/*bMustHaveHalfHP*/, true/*bMustBeRangedUnit*/, iDistance, !bHaveLOS, false/*bMustBeMeleeUnit*/, true/*bIgnoreUnits*/, pTargetPlot, 1 /*iMaxUnits*/))
#else
...
bool CvTacticalAI::ExecuteOneProtectedBombard(CvTacticalTarget& kTarget)
{
...
#if defined(MOD_BALANCE_CORE_MILITARY)
								if(FindClosestUnit(pAttackPlot, 0, false/*bMustHaveHalfHP*/, true /*bMustBeRangedUnit*/, iPlotDistance, !bHaveLOS, false/*bMustBeMeleeUnit*/,  false/*bIgnoreUnits*/, pTargetPlot, 1 /*iMaxUnits*/))
								{
									iNumTurns = 0;
								}
								// What about next turn?
								else if (FindClosestUnit(pAttackPlot, 1, false/*bMustHaveHalfHP*/, true /*bMustBeRangedUnit*/, iPlotDistance, !bHaveLOS, false/*bMustBeMeleeUnit*/, false/*bIgnoreUnits*/, pTargetPlot, 1 /*iMaxUnits*/))
								{
									iNumTurns = 1;
								}
#else
	...
	bool bHaveLOS = pAttackPlot->canSeePlot(pTargetPlot, m_pPlayer->getTeam(), iRange, NO_DIRECTION);
#if defined(MOD_BALANCE_CORE)
	if(FindClosestUnit(pAttackPlot, 0, false/*bMustHaveHalfHP*/, true /*bMustBeRangedUnit*/, iPlotDistance, !bHaveLOS, false/*bMustBeMeleeUnit*/,  false/*bIgnoreUnits*/, pTargetPlot, 1) ||
		FindClosestUnit(pAttackPlot, 1, false/*bMustHaveHalfHP*/, true /*bMustBeRangedUnit*/, iPlotDistance, !bHaveLOS, false/*bMustBeMeleeUnit*/,  false/*bIgnoreUnits*/, pTargetPlot, 1))
#else
	if(FindClosestUnit(pAttackPlot, 0, false/*bMustHaveHalfHP*/, true /*bMustBeRangedUnit*/, iPlotDistance, !bHaveLOS, false/*bMustBeMeleeUnit*/,  false/*bIgnoreUnits*/, pTargetPlot) ||
	...
	if(m_OperationUnits.size() > 0)
	{
		...
		for(unsigned int iI = 0; iI < m_TempTargets.size() && !bDone; iI++)
		{
			...
			if (eTargetType == AI_TACTICAL_TARGET_HIGH_PRIORITY_UNIT)
			{
				CvPlot* pLoopPlot = GC.getMap().plot(m_TempTargets[iI].GetTargetX(), m_TempTargets[iI].GetTargetY());
#if defined(MOD_BALANCE_CORE_MILITARY)
				if (FindClosestOperationUnit(pLoopPlot, true /*bIncludeRanged*/, true /*bMustBeRangedUnit*/, 5, 25, 3))
#else
				if (FindClosestOperationUnit(pLoopPlot, true /*bSafeForRanged*/, true /*bMustBeRangedUnit*/))
#endif
				{
		...
		for(unsigned int iI = 0; iI < m_TempTargets.size() && !bDone; iI++)
		{
			...
			{
				CvPlot* pLoopPlot = GC.getMap().plot(m_TempTargets[iI].GetTargetX(), m_TempTargets[iI].GetTargetY());
				if(FindClosestOperationUnit(pLoopPlot, true /*bIncludeRanged*/, false /*bMustBeRangedUnit*/, 5, 25, 3))
#else
			AITacticalTargetType eTargetType = m_TempTargets[iI].GetTargetType();
			if (eTargetType != AI_TACTICAL_TARGET_HIGH_PRIORITY_UNIT)
			{
...
CvPlot* CvTacticalAI::GetBestRepositionPlot(UnitHandle pUnit, CvPlot* plotTarget, int iAcceptableDanger)
{
	...
	for (TacticalAIHelpers::ReachablePlotSet::iterator moveTile=reachablePlots.begin(); moveTile!=reachablePlots.end(); ++moveTile)
	{
		...
		//already occupied?
		if (!pUnit->canMoveInto(*pMoveTile,CvUnit::MOVEFLAG_DESTINATION | CvUnit::MOVEFLAG_PRETEND_CORRECT_EMBARK_STATE))
			continue;
...
bool CvTacticalAI::FindUnitsWithinStrikingDistance2(CvPlot* pTarget, int iMinHitpoints, bool bNoRangedUnits, bool bNavalOnly, bool bMustMoveThrough, bool bIncludeBlockedUnits)
{
	...
	for(it = m_CurrentTurnUnits.begin(); it != m_CurrentTurnUnits.end(); it++)
	{
		...
		if(pLoopUnit)
		{
			...
			if (plotDistance(*pLoopUnit->plot(),*pTarget)>12)
				continue;
			...
			if ( (pDefender || bIsCityTarget) && pLoopUnit->IsCanAttackRanged() )
			{
				...
				else if (pLoopUnit->canMove() && pLoopUnit->getDomainType()!=DOMAIN_AIR)
				{
					...
					for (std::vector<CvPlot*>::iterator it=vPlots.begin(); it!=vPlots.end(); ++it)
					{
						...
						iTurnsToReach = TurnsToReachTarget(pLoopUnit, *it, false /*bReusePaths*/, false /*bIgnoreUnits*/, false /*bIgnoreStacking*/, 2);
			...
			//no path ... but if distance is low, take it nevertheless
			...
			if(pLoopUnit->IsCanAttackRanged())
			{
				...
				if(IsExpectedToDamageWithRangedAttack(pLoopUnit, pTarget, MIN(iTargetHitpoints/20, 3)))
				{
					...
					else if (bIncludeBlockedUnits && iTurnsToReach==2 && pLoopUnit->getDomainType()!=DOMAIN_AIR)
					{
						CvTacticalUnit unit;
						unit.SetID(pLoopUnit->GetID());
						if (bIsCityTarget)
...
bool CvTacticalAI::FindClosestOperationUnit(CvPlot* pTarget, bool bIncludeRanged, bool bRangedOnly, int iMaxTurns, int iMinHitpoints, int iMaxUnits)
{
	bool rtnValue = false;
	m_CurrentMoveUnits.clear();
...
bool CvTacticalAI::MoveToEmptySpaceNearTarget(UnitHandle pUnit, CvPlot* pTarget, bool bLand, int iMaxTurns)
{
	...
	//we include the target itself as well and iterate backwards!
	for(int iI=NUM_DIRECTION_TYPES; iI>-1; iI--)
	{
...
bool CvTacticalAI::MoveToEmptySpaceTwoFromTarget(UnitHandle pUnit, CvPlot* pTarget, bool bLand, int iMaxTurns)
{
	// Look at spaces adjacent to target
			...
			plotDistance(pLoopPlot->getX(), pLoopPlot->getY(), pTarget->getX(), pTarget->getY() == 2))
		{
			if(pUnit->canMoveInto(*pLoopPlot, CvUnit::MOVEFLAG_DESTINATION | CvUnit::MOVEFLAG_PRETEND_CORRECT_EMBARK_STATE))
			{
				...
				if(pLoopPlot->getPlotCity() == NULL || pLoopPlot->isFriendlyCity(*pUnit, false))
				{
					...
					if (CanReachInXTurns(pUnit,pLoopPlot,iMaxTurns,false))
					{
...
#if defined(MOD_BALANCE_CORE)
						//we check all plots and use the one with the lowest danger, even if it's more pathfinding
						int iTurns = TurnsToReachTarget(pUnit,pLoopPlot,true,false,false,iBestTurns);
						if (iTurns<iBestTurns)
						{
							iBestTurns = iTurns;
	...
	else
	{
		...
		for(int iI = 0; iI < NUM_DIRECTION_TYPES; iI++)
		{
			...
			if(pLoopPlot != NULL && !pLoopPlot->isWater())
			{
				...
				{
#if defined(MOD_BALANCE_CORE)
					if (!FindClosestUnit(pLoopPlot, 1, false/*bMustHaveHalfHP*/, false /*bMustBeRangedUnit*/, 2 /*iRangeRequired*/, false /*bNeedsIgnoreLOS*/, true /*bMustBeMeleeUnit*/, false, NULL, 3))
#else
...
{
	if(pLoopPlot == NULL || pLoopPlot->isImpassable(m_pPlayer->getTeam()) || !pLoopPlot->isRevealed(m_pPlayer->getTeam()))
		return 0;
...
bool TacticalAIHelpers::PerformAttack(CvUnit* pUnit, CvPlot* pTargetPlot)
{
	...
	if (pUnit->isRanged() && pUnit->getDomainType()!=DOMAIN_AIR)
	{
		...
		for (TacticalAIHelpers::ReachablePlotSet::iterator it=reachablePlots.begin(); it!=reachablePlots.end(); ++it)
		{
			...
			if (!pUnit->canMoveInto(*pCandidate,CvUnit::MOVEFLAG_DESTINATION | CvUnit::MOVEFLAG_PRETEND_CORRECT_EMBARK_STATE))
				continue;
...
CvPlot* TacticalAIHelpers::FindSafestPlotInReach(CvUnit* pUnit, bool bAllowEmbark)
{
	...
	TacticalAIHelpers::GetAllPlotsInReach(pUnit, pUnit->plot(), eligiblePlots, true, true);
	...
	for (TacticalAIHelpers::ReachablePlotSet::iterator it=eligiblePlots.begin(); it!=eligiblePlots.end(); ++it)
	{
		...
		if(!pUnit->canMoveInto(*pPlot,CvUnit::MOVEFLAG_DESTINATION | CvUnit::MOVEFLAG_PRETEND_CORRECT_EMBARK_STATE))
			continue;
...
CvPlot* TacticalAIHelpers::FindClosestSafePlotForHealing(CvUnit* pUnit, bool bWithinOwnTerritory, int iMaxDistance)
{
	...
	std::vector<SPlotWithScore> thisRing( 1, SPlotWithScore(pUnit->plot(),0) );
	std::vector<SPlotWithScore> nextRing;

	//work outwards in rings
	for (int iD=0; iD<iMaxDistance; iD++)
	{
		//check all neighbors which haven't been checked before
		for (std::vector<SPlotWithScore>::iterator it = thisRing.begin(); it!=thisRing.end(); ++it)
		{
			CvPlot** aPlotsToCheck = GC.getMap().getNeighborsUnchecked(it->pPlot);
			for(int iCount=0; iCount<NUM_DIRECTION_TYPES; iCount++)
			{
				CvPlot* pAdjacentPlot = aPlotsToCheck[iCount];
				if (!pAdjacentPlot)
				...
				nextRing.push_back( SPlotWithScore( pAdjacentPlot, GET_PLAYER(pUnit->getOwner()).GetCityDistance(pAdjacentPlot) ) );
