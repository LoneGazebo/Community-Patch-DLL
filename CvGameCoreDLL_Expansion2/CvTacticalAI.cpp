...
CvTactPosStorage gTactPosStorage(23000);
...
void ScoreAttack(const CvTacticalPlot& tactPlot, const CvUnit* pUnit, const CvTacticalPlot& assumedPlot, eAggressionLevel eAggLvl, float fAggBias, CAttackCache& cache, STacticalAssignment& result)
{
	...
	if (iDamageReceived > 0)
	{
		...
		if (bBelowHpLimitAfterMeleeAttack && (iScaledDamage - iDamageReceived) < 0)
		{
			result.iScore = -INT_MAX;
			return;
		}
	...
	*/
	...
	for (vector<STacticalAssignment>::iterator itMove = gOverAllChoices.begin(); itMove != gOverAllChoices.end(); ++itMove)
	{
		...
		else
		{
			...
			vector<STacticalUnit> blocks = findBlockingUnitsAtPlot(itMove->iToPlotIndex, *itMove);
			set<int> blockMoveToPlots;
			for (vector<STacticalUnit>::iterator itBlock = blocks.begin(); itBlock != blocks.end(); ++itBlock)
			{
				...
				for (vector<STacticalAssignment>::iterator itMove2 = blockingUnitChoices.begin(); itMove2 != blockingUnitChoices.end(); ++itMove2)
				{
					if (itMove2->eAssignmentType == A_MOVE && blockMoveToPlots.find(itMove2->iToPlotIndex)==blockMoveToPlots.end())
					{
						...
						{
							const STacticalAssignment* pLA = getLatestAssignment(blocks[0].iUnitID);
...
{ 
	...
	for (size_t i=0; i<notQuiteFinishedUnits.size(); i++)
	{
		...
		if (!bHaveException && iNextTurnScore < 1 && pInitial->iScore>TACTICAL_COMBAT_IMPOSSIBLE_SCORE)
			return false;
	...
	//second chance: did we attack anything (offensive) or at least improve the arrangement of our units (defensive)?
	//order is important here, need to add finish moves first!
	if (isAttackOrImprovedPosition())
	{
		notQuiteFinishedUnits.clear();
		return true;
	}
...
const CvTacticalPosition* tacticalPositionIsEquivalent(const CvTacticalPosition* ref, const CvTacticalPosition* other)
{
	...
	//trivial case
	if (ref->assignedMoves.size() != other->assignedMoves.size())
		return NULL;
