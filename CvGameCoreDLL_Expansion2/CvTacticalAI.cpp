		...
		if ( pLoopUnit->IsCanAttackRanged() )
		{
			...
			else if (pLoopUnit->canMove() && pLoopUnit->getDomainType()!=DOMAIN_AIR)
			{
				...
				ReachablePlots reachablePlots = TacticalAIHelpers::GetAllPlotsInReachThisTurn(pLoopUnit, pLoopUnit->plot(),
					CvUnit::MOVEFLAG_IGNORE_STACKING | CvUnit::MOVEFLAG_NO_EMBARK, bImmediateStrike?0:1);
...
void ScoreAttack(const CvTacticalPlot& tactPlot, CvUnit* pUnit, const CvTacticalPlot& assumedPlot, eAggressionLevel eAggLvl, float fAggBias, STacticalAssignment& result)
{
	...
	float fAggFactor = GC.getCOMBAT_AI_DAMAGEWEIGHT() / 100.f;
...
STacticalAssignment ScorePlotForCombatUnitOffensive(const SUnitStats unit, SMovePlot plot, const CvTacticalPosition& assumedPosition, bool bEndTurn)
{
	...
	if (currentPlot.isEnemy() && !pUnit->isRanged())
	{
		...
		if (iMaxAttacks > 0)
		{
			...
			if (iDistance == 1)
			{
				...
				if (result.eAssignmentType == A_MELEEKILL && currentPlot.getNumAdjacentEnemies()>3)
					return result;
				...
				//combo bonus
				if (result.eAssignmentType == A_MELEEKILL && currentPlot.isEnemyCivilian())
					result.iScore += 5;
	...
	else //empty plot or friendly unit. the latter case will be handled by the isMoveBlockedByOtherUnit check
	{
		...
		{
			//performance optimization
			if (iRange == 1 && currentPlot.getType(eRelevantDomain) != CvTacticalPlot::TP_FRONTLINE)
				continue;
			if (iRange == 2 && currentPlot.getType(eRelevantDomain) != CvTacticalPlot::TP_FRONTLINE && currentPlot.getType(eRelevantDomain) != CvTacticalPlot::TP_SECONDLINE)
				continue;
		...
		if (bEndTurn) //can happen if we run out of movement points or if we actively decide to stay in a plot
		{
			if (plot.iPlotIndex == unit.iPlotIndex)
			{
				...
				if (pUnit->shouldPillage(pCurrentPlot, false, plot.iMovesLeft) && !assumedPosition.unitHasAssignmentOfType(unit.iUnitID, A_PILLAGE))
				{
			...
			if (iDanger > 0)
			{
				...
				iDangerScore = (iDanger * GC.getCOMBAT_AI_DANGERWEIGHT()) / iSpreadFactor / (pUnit->GetCurrHitPoints() - unit.iSelfDamage + 1);
			}
...
bool CvTacticalPlot::hasSupportBonus() const
{
	if (nSupportUnitsAdjacent > 0)
		return true;
	...
	for (int i = 0; i < 6; i++)
	{
		...
		if (pNeighbor)
		{
			...
			if (tactPlot.isValid())
			{
				...
				if (tactPlot.hasFriendlyCombatUnit() || tactPlot.getType(eDomain)==TP_BLOCKED_FRIENDLY)
				...
				if (tactPlot.getType(eDomain) == TP_FRONTLINE && (tactPlot.hasFriendlyCombatUnit() || tactPlot.getType(eDomain)==TP_BLOCKED_FRIENDLY))
...
bool CvTacticalPosition::makeNextAssignments(int iMaxBranches, int iMaxChoicesPerUnit, CvTactPosStorage& storage)
{
	...
	*/
	...
	for (vector<SUnitStats>::iterator itUnit = availableUnits.begin(); itUnit != availableUnits.end(); ++itUnit)
	{
		...
		currentPlot.changeNeighboringUnitCount(*this, dummy, -1);
...
vector<STacticalAssignment> TacticalAIHelpers::FindBestDefensiveAssignment(const vector<CvUnit*>& vUnits, CvPlot* pTarget, CvTactPosStorage& storage)
{
	...
	*/
	...
	//now associate the remaining units with their plots
