...
if not VP and not (MapModData and MapModData.VP) then
	...
	if not iNumYields then
		...
		iNumYields = CPK.Misc.CustomModOptionEnabled("BALANCE_CORE_JFD") and iNumJFDYields or iNumCoreYields;
	end
	...
	--- This is a lot faster than using the `GameInfo` iterator.<br>
	...
	local _VP = setmetatable({}, {
		__index = function (t, key)
			...
			if key == "GameInfoTypes" then
				...
				for row in DB.Query([[select tbl_name from sqlite_master where type = 'table' and exists
						select 1 from pragma_table_info(s.tbl_name) as pti where pti.name = 'ID'
					) and exists (
						select 1 from pragma_table_info(s.tbl_name) as pti where pti.name = 'Type'
					)
				]]) do
