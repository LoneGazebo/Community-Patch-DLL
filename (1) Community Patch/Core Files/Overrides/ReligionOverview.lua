...
function RefreshYourReligion()
	...
	-- Set up disable automatic purchase checkbox
	local bDisableAutoPurchase = player:IsDisableAutomaticFaithPurchase();
	Controls.DisableAutoPurchaseCheckbox:SetCheck(bDisableAutoPurchase);
	Controls.DisableAutoPurchaseCheckbox:RegisterCheckHandler(function(bIsChecked)
		local player = Players[Game.GetActivePlayer()];
		player:DoSetDisableAutomaticFaithPurchase(bIsChecked);
	end);
