        ...
        if( not isHide ) then		
            ...
            if( g_bEnableThirdParty ) then
            	...
            	for iLoopPlayer = 0, GameDefines.MAX_CIV_PLAYERS-1, 1 do
					...
					if (iLoopTeam ~= g_iUsTeam and pLoopPlayer:IsAlive() and pLoopPlayer:IsMajorCiv()) then
						g_iNumOtherMajors = g_iNumOtherMajors + 1;
					end
...
function DoUpdateButtons()
    ...
    if (g_bPVPTrade) then
		...
		if (not g_pUs:IsDenouncedPlayer(g_iThem)) then
			...
			Controls.DenounceButton:SetText(Locale.ConvertTextKey("TXT_KEY_BUTTON_DENOUNCE_HUMAN"));
			Controls.DenounceButton:SetToolTipString(Locale.ConvertTextKey("TXT_KEY_BUTTON_DENOUNCE_HUMAN_TT"));
		end
...
function ResetDisplay()
	...
	local bShowResearchAgreement = (not bTeammates) and (not g_bDisableScience);
	...
	if bShowDefensivePact then
		...
		if iLimit >= g_iNumOtherMajors then
