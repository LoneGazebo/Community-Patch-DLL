...
void CvTeamTechs::Read(FDataStream& kStream)
{
	...
	else
	{
		...
		if(m_pTechs != NULL)
		{
			...
			CvInfosSerializationHelper::ReadV0DataArray(kStream, m_pabHasTech, m_pTechs->GetNumTechs(), CVINFO_V0_TAGS(ms_V0ExpansionTechTags));

			//ArrayWrapper<bool> kNoTradeTechWrapper(m_pTechs->GetNumTechs(), m_pabNoTradeTech);
			//kStream >> kNoTradeTechWrapper;
			CvInfosSerializationHelper::ReadV0DataArray(kStream, m_pabNoTradeTech, m_pTechs->GetNumTechs(), CVINFO_V0_TAGS(ms_V0ExpansionTechTags));

			//ArrayWrapper<int> kResearchProgressWrapper(m_pTechs->GetNumTechs(), m_paiResearchProgress);
			//kStream >> kResearchProgressWrapper;
			CvInfosSerializationHelper::ReadV0DataArray(kStream, m_paiResearchProgress, m_pTechs->GetNumTechs(), CVINFO_V0_TAGS(ms_V0ExpansionTechTags));

			//ArrayWrapper<int> kTechCountWrapper(m_pTechs->GetNumTechs(), m_paiTechCount);
			//kStream >> kTechCountWrapper;
			CvInfosSerializationHelper::ReadV0DataArray(kStream, m_paiTechCount, m_pTechs->GetNumTechs(), CVINFO_V0_TAGS(ms_V0ExpansionTechTags));
		}
	}
}
