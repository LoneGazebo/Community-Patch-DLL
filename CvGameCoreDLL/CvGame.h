...
public:
	...
	void CityPurchase(CvCity* pCity, UnitTypes eUnitType, BuildingTypes eBuildingType, ProjectTypes eProjectType);
	...
	void SetNumVictoryVotesTallied(int iValue);
	void ChangeNumVictoryVotesTallied(int iChange);
	...
	void SetVoteCast(TeamTypes eVotingTeam, TeamTypes eVote);
	...
	bool IsWaitingForBlockingInput() const {return (m_eWaitDiploPlayer != NO_PLAYER);};
	void SetWaitingForBlockingInput (PlayerTypes ePlayer) {m_eWaitDiploPlayer = ePlayer;};
