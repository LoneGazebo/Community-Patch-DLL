...
void CvAdvisorCounsel::BuildCounselList (PlayerTypes ePlayer)
{
	...
	for (uint ui = MAX_MAJOR_CIVS; ui < MAX_CIV_PLAYERS; ui++)
	{
		...
		if (pMinorCivAI->IsWarQuestWithAnyoneActive())
		{
			for (uint uiMajors = 0; uiMajors < MAX_MAJOR_CIVS; uiMajors++)
			{
				PlayerTypes eMajor = (PlayerTypes)ui;
				// he wants to be at war with the player!
				if (eMajor == ePlayer)
				{
					continue;
				}

				if (pMinorCivAI->IsWarQuestWithMajorActive(eMajor))
				{
					eEnemyPlayer = eMajor;
					break;
				}
		...
		if (pMinorCivAI->IsAllies(ePlayer) && pMinorCivAI->IsCloseToNotBeingAllies(ePlayer))
		{
			...
			{
				int iQuestData = pMinorCivAI->GetQuestData1(ePlayer);
...
bool CvAdvisorCounsel::SetCounselEntry(uint uiIndex, AdvisorTypes eAdvisor, const char* strAdvisorCounsel, int iValue)
{
	...
	strcpy(m_aCounsel[uiIndex].m_strTxtKey, strAdvisorCounsel);
