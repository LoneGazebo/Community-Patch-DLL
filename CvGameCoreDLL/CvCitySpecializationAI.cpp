...
void CvCitySpecializationAI::AssignSpecializations()
{
	...
	else
	{
		...
		int iCurrentDelta;
		int iBestDelta[NUM_YIELD_TYPES];
		...
		CvWeightedVector<int, NUM_YIELD_TYPES, true> yieldImprovements;
		for (iI = 0; iI < NUM_YIELD_TYPES; iI++)
		{
...
void CvCitySpecializationAI::SelectSpecializations()
{
	...
	if(pkWonderBuildCity && pkWonderBuildCity->getProductionBuilding() != NO_BUILDING)
	{
	...
	if (!m_bInterruptWonders && pkProductionBuildingInfo && m_pPlayer->GetWonderProductionAI()->IsWonder(*pkProductionBuildingInfo))
	{
	...
	else if (m_eNextWonderDesired != NO_BUILDING)
	{
		for (pLoopCity = m_pPlayer->firstCity(&iLoop); pLoopCity != NULL; pLoopCity = m_pPlayer->nextCity(&iLoop))
		{
			if (!pLoopCity->IsPuppet())
			{
				...
				{
					iProduction = pLoopCity->getProduction();
...
void CvCitySpecializationAI::LogSpecializationWeights()
{
	if (GC.getLogging() && GC.getAILogging())
	{
		...
		for (int iI = 0; iI < NUM_SPECIALIZATION_YIELDS; iI++)
		{
			...
			{
				strYieldString = GC.getYieldInfo((YieldTypes)m_YieldWeights.GetElement(iI))->GetDescription();
			}
...
void CvCitySpecializationAI::LogSpecializationUpdate(CitySpecializationUpdateType eUpdate)
{
	if (GC.getLogging() && GC.getAILogging())
	{
		...
		case SPECIALIZATION_UPDATE_NEW_GRAND_STRATEGY:
			strTypeString.Format ("Update: New grand strategy: %s", GC.getAIGrandStrategyInfo(m_pPlayer->GetGrandStrategyAI()->GetActiveGrandStrategy())->GetType());
			break;
		case SPECIALIZATION_UPDATE_NOW_AT_WAR:
...
void CvCitySpecializationAI::LogBestSites()
{
	if (GC.getLogging() && GC.getAILogging())
	{
		...
		for (int iI = 0; iI < NUM_YIELD_TYPES; iI++)
		{
			strYieldString = GC.getYieldInfo((YieldTypes)iI)->GetDescription();
