...
bool CvTreasury::HasRouteBetweenCities (CvCity* pFirstCity, CvCity* pSecondCity, bool bBestRoute) const
{
	...
	FASSERT(iFirstCityIndex >= 0, "first city not found");
	FASSERT(iSecondCityIndex >= 0, "second city not found");
	if (iFirstCityIndex < 0 || iSecondCityIndex < 0)
	{
...
int CvTreasury::CalculateUnitCost(int& iFreeUnits, int& iPaidUnits, int& iBaseUnitCost, int& iExtraCost)
{
	...
	if (iLandUnitMod != 0)
	{
		...
		iBaseUnitCost += (iLandUnitMod * iLandUnits * m_pPlayer->getGoldPerUnitTimes100()) / 100;
	}
	...
	if (iNavalUnitMod != 0)
	{
		...
		iBaseUnitCost += (iNavalUnitMod * iNavalUnits * m_pPlayer->getGoldPerUnitTimes100()) / 100;
	}
