...
print("Loading EUI tech tree...",os.clock(),[[ 
 _____         _   _____              
...
include( "TechButtonInclude" ); local IconHookup = IconHookup; local GatherInfoAboutUniqueStuff = GatherInfoAboutUniqueStuff; local AddSmallButtonsToTechButton = AddSmallButtonsToTechButton
include( "TechHelpInclude" ); local GetHelpTextForTech = GetHelpTextForTech
...
local function setTechToolTip( techID, void2, button )
...
function( popupInfo )
	...
	if g_popupInfoType then
		...
		if Controls.TechTreeScrollPanel:GetScrollValue() == 0 then
			...
			if x then
				Controls.TechTreeScrollPanel:SetScrollValue( math.min(1,math.max(0,( (x + dx)*g_blockSpacingX/Controls.TechTreeScrollPanel:GetSizeX() - 0.5) / math.max(1,1/Controls.TechTreeScrollPanel:GetRatio() - 1) ) ) )
			end
		end
