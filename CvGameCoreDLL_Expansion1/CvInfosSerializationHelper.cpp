...
{
	return Write(kStream, (const CvBaseInfo*)GC.getBeliefInfo(eType));
}
...
{
	if(eType != NO_BELIEF)
		return WriteHashed(kStream, (const CvBaseInfo*)GC.getBeliefInfo(eType));
	else
		return WriteHashed(kStream, (const CvBaseInfo*)NULL);
}
...
{
	return Write(kStream, (const CvBaseInfo*)GC.getReligionInfo(eType));
}
...
{
	if(eType != NO_BELIEF)
		return WriteHashed(kStream, (const CvBaseInfo*)GC.getReligionInfo(eType));
	else
		return WriteHashed(kStream, (const CvBaseInfo*)NULL);
}
