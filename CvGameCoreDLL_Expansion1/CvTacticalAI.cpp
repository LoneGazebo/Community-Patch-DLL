...
bool CvTacticalAI::PlotCaptureCityMoves()
{
	...
	while(pTarget != NULL)
	{
		...
		if(pPlot != NULL)
		{
			...
			if(FindUnitsWithinStrikingDistance(pPlot, 1, false /*bNoRangedUnits*/, false /*bNavalOnly*/, false /*bMustMoveThrough*/, true /*bIncludeBlockedUnits*/))
			{
...
void CvTacticalAI::PlotBarbarianCampMoves()
{
	...
	while(pTarget != NULL)
	{
		// See what units we have who can reach target this turn
		CvPlot* pPlot = GC.getMap().plot(pTarget->GetTargetX(), pTarget->GetTargetY());
		if(FindUnitsWithinStrikingDistance(pPlot, 1, false /* bNoRangedUnits */, false /*bNavalOnly*/, false /*bMustMoveThrough*/))
		{
...
void CvTacticalAI::PlotPillageMoves()
{
	...
	while(pTarget != NULL)
	{
		// See what units we have who can reach target this turn
		CvPlot* pPlot = GC.getMap().plot(pTarget->GetTargetX(), pTarget->GetTargetY());
		if(FindUnitsWithinStrikingDistance(pPlot, 0, false /* bNoRangedUnits */, false /*bNavalOnly*/, true /*bMustMoveThrough*/, false /*bIncludeBlockedUnits*/, true /*bWillPillage*/))
		{
			// Queue best one up to capture it
		...
		else if(FindUnitsWithinStrikingDistance(pPlot, 2, false /* bNoRangedUnits */, false /*bNavalOnly*/, false /*bMustMoveThrough*/, false /*bIncludeBlockedUnits*/, true /*bWillPillage*/))
		{
...
void CvTacticalAI::PlotBlockadeImprovementMoves()
{
	...
	while(pTarget != NULL)
	{
		// See what units we have who can reach target this turn
		CvPlot* pPlot = GC.getMap().plot(pTarget->GetTargetX(), pTarget->GetTargetY());
		if(FindUnitsWithinStrikingDistance(pPlot, 2, false /* bNoRangedUnits */, true /* bNavalOnly */, false /*bMustMoveThrough*/))
		{
			// Queue best one up to capture it
...
void CvTacticalAI::PlotCampDefenseMoves()
{
	...
	{
		CvPlot* pPlot = GC.getMap().plot(pTarget->GetTargetX(), pTarget->GetTargetY());
		if(FindUnitsWithinStrikingDistance(pPlot, 1, true /* bNoRangedUnits */, false /*bNavalOnly*/, false /*bMustMoveThrough*/))
		{
...
void CvTacticalAI::ExecuteCloseOnTarget(CvTacticalTarget& kTarget, CvTacticalDominanceZone* pZone)
{
	...
	if(m_OperationUnits.size() > 0)
	{
		ScoreCloseOnPlots(pTargetPlot, !pZone->IsNavalInvasion() /* Land only unless invasion */);
...
bool CvTacticalAI::FindUnitsWithinStrikingDistance(CvPlot* pTarget, int iNumTurnsAway, bool bNoRangedUnits, bool bNavalOnly, bool bMustMoveThrough, bool bIncludeBlockedUnits, bool bWillPillage)
{
	list<int>::iterator it;
	UnitHandle pLoopUnit;
	...
	for(it = m_CurrentTurnUnits.begin(); it != m_CurrentTurnUnits.end(); it++)
	{
		...
		if(pLoopUnit)
		{
			if(!bNavalOnly || pLoopUnit->getDomainType() == DOMAIN_SEA)
			{
				...
				if(!bNoRangedUnits && !bWillPillage && pLoopUnit->IsCanAttackRanged())
				{
				...
				else
				{
					...
					if (bMustMoveThrough && CanReachInXTurns(pLoopUnit, pTarget, 0, false /*bIgnoreUnits*/, &iTurnsCalculated))
					{
						CvTacticalUnit unit;
						unit.SetID(pLoopUnit->GetID());
					...
					else if (bIncludeBlockedUnits && CanReachInXTurns(pLoopUnit, pTarget, iNumTurnsAway, true /*bIgnoreUnits*/))
					{
						CvTacticalUnit unit;
						unit.SetID(pLoopUnit->GetID());
						unit.SetAttackStrength(pLoopUnit->GetMaxAttackStrength(NULL, NULL, NULL) / 2);
						unit.SetHealthPercent(pLoopUnit->GetCurrHitPoints(), pLoopUnit->GetMaxHitPoints());
						m_CurrentMoveUnits.push_back(unit);
						rtnValue = true;
					}
