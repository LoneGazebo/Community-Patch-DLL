...
-- LAND
	...
	UPDATE Units SET PrereqTech = 'TECH_RIFLING' WHERE Type = 'UNIT_FRENCH_FOREIGNLEGION' AND EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_UNITS' AND Value= 1 );
...
--	INSERT INTO Unit_FreePromotions (UnitType, PromotionType) SELECT 'UNIT_ZEPPELIN', 'PROMOTION_CAN_MOVE_AFTER_ATTACKING' WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_UNITS' AND Value= 1 );
	...
	UPDATE Unit_AITypes SET UnitAIType = 'UNITAI_DEFENSE' WHERE UnitType = 'UNIT_MECH' AND UnitAIType = "UNITAI_ATTACK" AND EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_UNITS' AND Value= 1 );
	...
	--Flavor Fixes
	-- air removed from recon
	DELETE FROM Unit_Flavors WHERE UnitType IN (SELECT Type FROM Units WHERE CombatClass = "UNITCOMBAT_RECON") AND FlavorType = 'FLAVOR_AIR';
	INSERT INTO Unit_Flavors (UnitType, FlavorType, Flavor) VALUES
	('UNIT_MARINE', 'FLAVOR_RECON', 10)
