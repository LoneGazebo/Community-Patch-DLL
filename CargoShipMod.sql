...
WHERE EXISTS (SELECT * FROM CustomModOptions WHERE Type='CARGO_SHIPS' AND Value= 1 );
...
WHERE Type IN (SELECT Type FROM Units WHERE CombatClass = 'UNITCOMBAT_MELEE' OR CombatClass = 'UNITCOMBAT_ARCHER' OR CombatClass = 'UNITCOMBAT_GUN' OR CombatClass = 'UNITCOMBAT_HELICOPTER' OR CombatClass	= 'UNITCOMBAT_MOUNTED' OR CombatClass = 'UNITCOMBAT_ARMOR' OR CombatClass = 'UNITCOMBAT_SIEGE' OR CombatClass = 'UNITCOMBAT_RECON') AND EXISTS (SELECT * FROM CustomModOptions WHERE Type='CARGO_SHIPS' AND Value= 1 );

UPDATE Units
SET SpecialCargo = 'SPECIALUNIT_CARGO_ARMY', DomainCargo = 'DOMAIN_LAND', CargoCombat = 20
WHERE Type IN (SELECT Type FROM Units WHERE CombatClass = 'UNITCOMBAT_NAVALMELEE') AND EXISTS (SELECT * FROM CustomModOptions WHERE Type='CARGO_SHIPS' AND Value= 1 );

UPDATE Language_en_US 
SET Text = Text||(SELECT ' '||Text FROM Language_en_US WHERE Tag = 'TXT_KEY_ARMY_CARGO_UNIT_1')
WHERE Tag IN (SELECT Help FROM Units WHERE SpecialCargo = 'SPECIALUNIT_CARGO_ARMY' AND Type IN (SELECT UnitType FROM Unit_FreePromotions WHERE PromotionType = 'PROMOTION_CARGO_I')) AND EXISTS (SELECT * FROM CustomModOptions WHERE Type='CARGO_SHIPS' AND Value= 1 );

UPDATE Language_en_US 
SET Text = Text||(SELECT ' '||Text FROM Language_en_US WHERE Tag = 'TXT_KEY_ARMY_CARGO_UNIT_2')
WHERE Tag IN (SELECT Help FROM Units WHERE SpecialCargo = 'SPECIALUNIT_CARGO_ARMY' AND Type IN (SELECT UnitType FROM Unit_FreePromotions WHERE PromotionType = 'PROMOTION_CARGO_II')) AND EXISTS (SELECT * FROM CustomModOptions WHERE Type='CARGO_SHIPS' AND Value= 1 );

UPDATE Language_en_US 
SET Text = Text||(SELECT ' '||Text FROM Language_en_US WHERE Tag = 'TXT_KEY_ARMY_CARGO_UNIT_3')
WHERE Tag IN (SELECT Help FROM Units WHERE SpecialCargo = 'SPECIALUNIT_CARGO_ARMY' AND Type IN (SELECT UnitType FROM Unit_FreePromotions WHERE PromotionType = 'PROMOTION_CARGO_III')) AND EXISTS (SELECT * FROM CustomModOptions WHERE Type='CARGO_SHIPS' AND Value= 1 );

UPDATE Language_en_US 
SET Text = Text||(SELECT ' '||Text FROM Language_en_US WHERE Tag = 'TXT_KEY_ARMY_CARGO_UNIT_4')
WHERE Tag IN (SELECT Help FROM Units WHERE SpecialCargo = 'SPECIALUNIT_CARGO_ARMY' AND Type IN (SELECT UnitType FROM Unit_FreePromotions WHERE PromotionType = 'PROMOTION_CARGO_IV')) AND EXISTS (SELECT * FROM CustomModOptions WHERE Type='CARGO_SHIPS' AND Value= 1 );

INSERT INTO Unit_FreePromotions
		(UnitType,	PromotionType)
SELECT	Type,		('PROMOTION_CARGO_I')
FROM Units WHERE (Class = 'UNITCLASS_TRIREME') AND EXISTS (SELECT * FROM CustomModOptions WHERE Type='CARGO_SHIPS' AND Value= 1 );

INSERT INTO Unit_FreePromotions
		(UnitType,	PromotionType)
SELECT	Type,		('PROMOTION_CARGO_II')
FROM Units WHERE (Class = 'UNITCLASS_CARAVEL' OR Class = 'UNITCLASS_PRIVATEER' OR Class = 'UNITCLASS_IRONCLAD') AND EXISTS (SELECT * FROM CustomModOptions WHERE Type='CARGO_SHIPS' AND Value= 1 );

INSERT INTO Unit_FreePromotions
		(UnitType,	PromotionType)
SELECT	Type,		('PROMOTION_CARGO_III')
FROM Units WHERE (Class = 'UNITCLASS_DESTROYER') AND EXISTS (SELECT * FROM CustomModOptions WHERE Type='CARGO_SHIPS' AND Value= 1 );
...
INSERT INTO Language_en_US
			(Tag,													Text)
SELECT		'TXT_KEY_PROMOTION_ARMY_SUPPORT',						'Pirate Ship'
SELECT		'TXT_KEY_PROMOTION_ARMY_SUPPORT_HELP',					'Before engaging in naval melee combat, this unit has an extra attack'
SELECT		'TXT_KEY_PROMOTION_ARMY_ON_SHIP',						'Military Unit on Ship'
SELECT		'TXT_KEY_PROMOTION_ARMY_ON_SHIP_HELP',					'Unit is cargo on and can embark or disembark from Ship to Land.'
SELECT		'TXT_KEY_SPECIALUNIT_CARGO_ARMY',						'Army Cargo Unit'
SELECT		'TXT_KEY_PROMOTION_ARMY_RANGE_PENALTY',					'Ranged Naval Penalty'
SELECT		'TXT_KEY_PROMOTION_ARMY_RANGE_PENALTY_HELP',			'-25% [ICON_STRENGTH] Ranged Strength and -1 Range when using Ranged Attacks'
SELECT		'TXT_KEY_PROMOTION_ARMY_SCOUT_SHIP',					'Scout Unit on Ship'
SELECT		'TXT_KEY_PROMOTION_ARMY_SCOUT_SHIP_HELP',				'+2 Visibility Range'
SELECT		'TXT_KEY_ARMY_CARGO_UNIT_1',							'[NEWLINE]Can carry 1 Melee, Archer, Siege, Mounted, Recon, Armor, Helicopter, or Gunpowder Units.'
SELECT		'TXT_KEY_ARMY_CARGO_UNIT_2',							'[NEWLINE]Can carry 2 Melee, Archer, Siege, Mounted, Recon, Armor, Helicopter, or Gunpowder Units.'
SELECT		'TXT_KEY_ARMY_CARGO_UNIT_3',							'[NEWLINE]Can carry 3 Melee, Archer, Siege, Mounted, Recon, Armor, Helicopter, or Gunpowder Units.'
