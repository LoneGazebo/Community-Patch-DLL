...
PopupLayouts[ButtonPopupTypes.BUTTONPOPUP_CITY_CAPTURED] = function(popupInfo)
	...
	if (iLiberatedPlayer ~= -1) then
		...
		if (Game.IsOption("GAMEOPTION_ENABLE_VASSALAGE") and not Game.IsOption("GAMEOPTION_NO_VOLUNTARY_VASSALAGE") and not Players[iLiberatedPlayer]:IsAlive() and not Players[iLiberatedPlayer]:IsMinorCiv()) then
