...
UPDATE Language_en_US
SET Text = 'The {TXT_KEY_UNIT_AMERICAN_MINUTEMAN} is very powerful as it requires 50% less expereince to unlock the Range Promotion, thanks to Barrage I. Combined with Ignoring Rough Terrain you can position many Minuteman quickly and strike Cities or Units many times while being safe from ranged retaliation. Make sure to finish off units with them to gain Golden Age Points. {TXT_KEY_UNIT_AMERICAN_MINUTEMAN} are also excellent in defense as they can retreat in Rough terrain and fire in the same turn.
...
UPDATE Language_en_US
SET Text = 'Completing a [ICON_CARAVAN]/[ICON_CARGO_SHIP] Trade Route targeting another Civilization triggers a (or strengthens an existing) [COLOR_POSITIVE_TEXT]Historic Event[ENDCOLOR].[NEWLINE][NEWLINE][ICON_CARAVAN] Land Trade Routes gain 50% range. Incoming [ICON_INTERNATIONAL_TRADE] Trade Routes generate +1 [ICON_GOLD] Gold for this City, and +1 [ICON_GOLD] Gold for the origin City.[NEWLINE][NEWLINE]Nearby [ICON_RES_SPICES] Cinnamon: +1 [ICON_FOOD] Food, +1 [ICON_PRODUCTION] Production.[NEWLINE]Nearby [ICON_RES_SUGAR] Sugar: +1 [ICON_FOOD] Food, +1 [ICON_GOLD] Gold.'
...
UPDATE Language_en_US
SET Text = 'Unique Arabian {TXT_KEY_UNIT_HEAVY_SKIRMISHER} that does not require [ICON_RES_HORSE] {TXT_KEY_RESOURCE_HORSES} and can withdraw from melee combat once per turn. Starts with [COLOR_POSITIVE_TEXT]{TXT_KEY_PROMOTION_SPLASH_1}[ENDCOLOR].'
...
UPDATE Language_en_US
SET Text = '+20% [ICON_RESEARCH] Science during [ICON_GOLDEN_AGE] Golden Ages. +1 [ICON_RESEARCH] Science for every 2 [ICON_CITIZEN] Citizens in the City, and all Libraries produce +3 [ICON_RESEARCH] Science. Comes with both a [COLOR_POSITIVE_TEXT]Free[ENDCOLOR] [ICON_GW_WRITING] Great Work of Writing and a [COLOR_POSITIVE_TEXT]Free[ENDCOLOR] {TXT_KEY_BUILDING_LIBRARY} in the City.[NEWLINE][NEWLINE]Every [ICON_GW_WRITING] Great Work of Writing you control grants +5 XP to all created Units (maximum of 45 XP).[NEWLINE][NEWLINE]-1 [ICON_HAPPINESS_3] Unhappiness from [ICON_RESEARCH] Illiteracy in all Cities. 1 Specialist in this City no longer produces [ICON_HAPPINESS_3] Unhappiness from [ICON_URBANIZATION] Urbanization.[NEWLINE][NEWLINE]Contains 3 slots for [ICON_GW_WRITING] Great Works of Writing. +3 [ICON_PRODUCTION] Production and [ICON_RESEARCH] Science if [COLOR_POSITIVE_TEXT]Themed[ENDCOLOR].[NEWLINE][NEWLINE]The [ICON_PRODUCTION] Production Cost increases based on the number of Cities you own.'
...
UPDATE Language_en_US
SET Text = 'The main bonus of the {TXT_KEY_CIV5_BABYLON_WALLS} really game is the +3 Science, which is more than Library, so it is worth building {TXT_KEY_CIV5_BABYLON_WALLS} quickly. You can aim to settle or conquer many Cities throughout the game to stack the bonus. In the late game the main benefit shifts to the 5% increase in Science generated by Great Scientists when used to research new Technologies. Expending Great Scienctists naturally becomes much more valuable than creating Academies in later stages of the game, and for Babylon this happens sooner. 
  [NEWLINE][NEWLINE]The significant boost to city defense is invaluable in frontier cities, which synergizes with the aggressive settling strategy as these vulnerable cities are easier to defend, especially with your Unique Units.'
...
UPDATE Language_en_US
SET Text = 'The {TXT_KEY_UNIT_BYZANTINE_CATAPHRACT} is a strong improvement over the already very strong Knight. It is very strong both in offense and defense. It will win most 1-on-1 fights until Lancers appear on the battlefield, so make sure to own as many horse resources as possible and build as many Cataphracts as you can, no matter if you plan to attack or only deter and defend. It is because a smaller, cheaper to maintain army of Cataphracts can be as strong as a larger army of Melee and Ranged units. The +30% Combat Strength bonus in Open terrain is stronger than defensive bonus in Rough terrain that Cataphracts can uniquely gain, so try to use them in Open terrain more, especially since they will be able to maneuver after attacking better. High Combat Strength encourages adding as many Combat Strength % bonuses as possible, so the already-strong Shock Promotion line is encouraged. In offense, make sure to flank Enemy Units before attacking to take full advantage of the flanking Combat Strength bonus.  Consider choosing Mobility Promotion over others as its worth more for Cataphracts than Knights, due to their lower Movement. In Defense, the ability to fortify synergises well with Aplekton Unique Improvement making Cataphracts excellent at protecting ranged units behind them.'
...
UPDATE Language_en_US
SET Text = 'Unique Carthaginian {TXT_KEY_UNIT_TRIREME} that specializes in naval assaults and capturing coastal cities. Starts with [COLOR_POSITIVE_TEXT]{TXT_KEY_PROMOTION_PINCER}[ENDCOLOR].'
...
UPDATE Language_en_US
SET Text = 'When you gain a City, one of 3 unique Luxuries ([ICON_RES_CLOVES]/[ICON_RES_PEPPER]/[ICON_RES_NUTMEG]) will appear nearby. +5% to Yield and [ICON_GOLDEN_AGE] Golden Age duration modifiers from [ICON_MONOPOLY] Global Monopolies; +2 to Yields and [ICON_HAPPINESS_1] Happiness from [ICON_MONOPOLY] Global Monopolies.'
...
UPDATE Language_en_US
SET Text = 'Unique Dutch {TXT_KEY_UNIT_PRIVATEER} that specializes in attacking coastal cities and capturing enemy ships. Starts with [COLOR_POSITIVE_TEXT]{TXT_KEY_PROMOTION_VANGUARD}[ENDCOLOR] and [COLOR_POSITIVE_TEXT]{TXT_KEY_PROMOTION_SUPPLY}[ENDCOLOR].'
...
UPDATE Language_en_US
SET Text = 'Unique Spanish {TXT_KEY_UNIT_EXPLORER} that specializes in conquest and settling advanced Cities overseas. Starts with [COLOR_POSITIVE_TEXT]{TXT_KEY_PROMOTION_OVERRUN}[ENDCOLOR] and [COLOR_POSITIVE_TEXT]{TXT_KEY_PROMOTION_VANGUARD}[ENDCOLOR].[NEWLINE][NEWLINE]Can be expended to found a City in any foreign landmass. Cities established by this Unit start with the same buildings as Cities established by a {TXT_KEY_UNIT_PIONEER} plus {TXT_KEY_BUILDING_LIGHTHOUSE} and {TXT_KEY_BUILDING_ARMORY}.'
...
UPDATE Language_en_US
SET Text = '+3 [ICON_RESEARCH] Science from Temples and Shrines in this City. +10 [ICON_INFLUENCE] Influence from Diplomatic Missions by Units originating from this City.[NEWLINE][NEWLINE]+25 [ICON_CITY_SECURITY] City Security, plus 1 for every two [ICON_CITIZEN] Citizens in the City.[NEWLINE][NEWLINE]-1 [ICON_HAPPINESS_3] Unhappiness from [ICON_FOOD] and [ICON_PRODUCTION] Distress.'
WHERE Tag = 'TXT_KEY_BUILDING_WAT_HELP';

UPDATE Language_en_US
SET Text = 'Unique Siamese replacement for the {TXT_KEY_BUILDING_CONSTABLE}. In addition to the {TXT_KEY_BUILDING_CONSTABLE}''s bonuses, {TXT_KEY_BUILDING_WAT_DESC} increases [ICON_CITY_SECURITY] City Security even more; generates [ICON_RESEARCH] Science, [ICON_CULTURE] Culture, and [ICON_PEACE] Faith; and boosts Shrines, Temples, and diplomatic units. It also has a [ICON_SCIENTIST] Scientist slot. It is available in the [COLOR_CYAN]Medieval Era[ENDCOLOR] with the discovery of [COLOR_CYAN]{TXT_KEY_TECH_THEOLOGY_TITLE}[ENDCOLOR], much earlier than the {TXT_KEY_BUILDING_CONSTABLE}.'
...
UPDATE Language_en_US
SET Text = 'The {TXT_KEY_UNIT_SIAMESE_WARELEPHANT} is very powerful when fighting melee units, so use it primarily for this. Move it adjacent to enemy units you plan to attack with other units so that the Feared Elephant Promotion weakens the target for your other units. Make sure to stack your Great Generals with {TXT_KEY_UNIT_SIAMESE_WARELEPHANT} to use the Combat Strength bonus.
  [NEWLINE][NEWLINE]Because it does not require Horse Resources you can build many of them, so {TXT_KEY_UNIT_SIAMESE_WARELEPHANT} should replace all of your melee Units because they are weaker in most scenarios. While 3 Movement is lower than standard 4 of mounted units, it still allows for good flexibility and focus attacks especially with the ability to move after attacking. You can use your unused Horses to build Skirmishers and flank Enemy Units before attacking with the {TXT_KEY_UNIT_SIAMESE_WARELEPHANT} to increase the flanking bonus massively. Therefore Shock promotion line is heavily encouraged for {TXT_KEY_UNIT_SIAMESE_WARELEPHANT}. Moreover, the Mobility Promotion becomes more valuable due to the lower movement speed of {TXT_KEY_UNIT_SIAMESE_WARELEPHANT}.'
