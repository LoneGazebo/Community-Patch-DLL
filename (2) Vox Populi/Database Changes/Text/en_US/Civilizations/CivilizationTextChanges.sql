...
UPDATE Language_en_US
SET Text = 'The {TXT_KEY_UNIT_ARABIAN_CAMELARCHER} can be used more aggressively since it can dodge a single melee attack per turn. Attack from tiles where the unit is adjacent to as many enemy units as possible to maximise the unique splash damage. The Parthian Tactics Promotion is useful for that purpose. Alternatively, Logistics allows to splash 2 times in one turn. Like with all ranged mounted units, Envelopment Promotion is a great choice too as it synergises with Skirmisher Doctrine, giving large flanking bonuses. [NEWLINE][NEWLINE] The {TXT_KEY_UNIT_ARABIAN_CAMELARCHER} is a very powerful unit that will upgrade to the {TXT_KEY_HASHEMITE_RAIDER}{TXT_KEY_CURIASSIER} though later in the game, so make them the main force of your army. You may be unable to upgrade them all to {TXT_KEY_CURIASSIER}s due to the deficit of [ICON_RES_HORSE] {TXT_KEY_RESOURCE_HORSES}, so consider unlocking {TXT_KEY_HASHEMITE_RAIDER}s fast as [ICON_RES_ALUMINUM] {TXT_KEY_RESOURCE_ALUMINUM} is more plentiful. [NEWLINE][NEWLINE]At the risk of being hated, consider declaring wars continously and sieging cities with {TXT_KEY_UNIT_ARABIAN_CAMELARCHER}s to achieve >25 Warscore, because then, a peace-treaty will trigger a Historic event, synergising with your Unique Ability.'
