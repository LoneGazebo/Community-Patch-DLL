...
UPDATE Language_en_US
SET Text = '[COLOR_CYAN]Special Improvement: Town[ENDCOLOR][NEWLINE]You can expend a Great Merchant to create a Town. The Town [COLOR_YELLOW]receives additional yields from being on routes[ENDCOLOR] and, while worked, produces a lot of [ICON_GOLD] Gold [COLOR_YELLOW]and [ICON_FOOD] Food.[ENCDOLOR][NEWLINE][NEWLINE][COLOR_CYAN]Special Ability: {TXT_KEY_MISSION_CONDUCT_TRADE_MISSION}[ENDCOLOR][NEWLINE]If the Great Merchant is inside [ICON_CITY_STATE] City-State territory that you are not at [ICON_WAR] War with, you can expend it to conduct a Trade Mission.[NEWLINE][NEWLINE]You will receive a large amount of [ICON_GOLD] Gold [COLOR_YELLOW]equal to that gained over the last ' || (SELECT BaseGoldTurnsToCount FROM Units WHERE Type = 'UNIT_MERCHANT') ||  ' turns, and an instant "We Love the King Day" in all owned cities for ' || (SELECT BaseWLTKDTurns FROM Units WHERE Type = 'UNIT_MERCHANT') ||' turns. Every Town you''ve created and own increases the Gold and length of the "We Love the King Day" by ' || (SELECT Amount FROM Unit_ScalingFromOwnedImprovements WHERE ImprovementType = 'IMPROVEMENT_CUSTOMS_HOUSE' AND UnitType = 'UNIT_MERCHANT') || '%.[ENDCOLOR]'
