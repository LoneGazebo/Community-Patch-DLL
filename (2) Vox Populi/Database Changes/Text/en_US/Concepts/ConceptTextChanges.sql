...
UPDATE Language_en_US
SET Text = '[COLOR_CYAN]Special Ability: {TXT_KEY_MISSION_CREATE_GREAT_WORK}[ENDCOLOR][NEWLINE]A Great Artist can create a [ICON_GW_ART] Great Work of Art (generates both [ICON_CULTURE] Culture and [ICON_TOURISM] Tourism) that is placed in the nearest city that has an appropriate building with an empty slot (like a Monument, a Gallery, or a Museum). The Great Artist is expended when used this way.[NEWLINE][NEWLINE][COLOR_CYAN]Special Ability: {TXT_KEY_MISSION_START_GOLDENAGE}[ENDCOLOR][NEWLINE]A Great Artist can be expended to [COLOR_YELLOW]generate Golden Age Points. The amount of Golden Age Points generated is based on the [ICON_TOURISM] Tourism and [ICON_GOLDEN_AGE] Golden Age Point output of the past ' || (SELECT BaseTurnsForGAPToCount FROM Units WHERE Type = 'UNIT_ARTIST') || ' turns, and is increased by ' || (SELECT ScaleFromNumThemes FROM Units WHERE Type = 'UNIT_ARTIST') || '% for every owned themed Great Work set.[ENDCOLOR]'
...
UPDATE Language_en_US
SET Text = '[COLOR_CYAN]Special Improvement: Academy[ENDCOLOR][NEWLINE]You can expend a Great Scientist to create an Academy. [COLOR_YELLOW]The Academy counts as every type of Improvement for adjacency bonuses[ENDCOLOR] and, while worked, will give your city a big [ICON_RESEARCH] Science boost.[NEWLINE][NEWLINE][COLOR_CYAN]Special Ability: {TXT_KEY_MISSION_DISCOVER_TECH}[ENDCOLOR][NEWLINE]You can expend your Great Scientist to immediately gain a lump sum of [ICON_RESEARCH] Science[COLOR_YELLOW] equal to that gained over the last ' || (SELECT BaseBeakersTurnsToCount FROM Units WHERE Type = 'UNIT_SCIENTIST') || ' turns. Every owned Academy you''ve created and own increases the amount of science gained by ' || (SELECT Amount FROM Unit_ScalingFromOwnedImprovements WHERE ImprovementType = 'IMPROVEMENT_ACADEMY' AND UnitType = 'UNIT_SCIENTIST') || '%.[ENDCOLOR]'
...
UPDATE Language_en_US
SET Text = '[COLOR_CYAN]Special Improvement: Town[ENDCOLOR][NEWLINE]You can expend a Great Merchant to create a Town. The Town [COLOR_YELLOW]receives additional yields from being on routes[ENDCOLOR] and, while worked, produces a lot of [ICON_GOLD] Gold [COLOR_YELLOW]and [ICON_FOOD] Food.[ENDCOLOR][NEWLINE][NEWLINE][COLOR_CYAN]Special Ability: {TXT_KEY_MISSION_CONDUCT_TRADE_MISSION}[ENDCOLOR][NEWLINE]If the Great Merchant is inside [ICON_CITY_STATE] City-State territory that you are not at [ICON_WAR] War with, you can expend it to conduct a Trade Mission.[NEWLINE][NEWLINE]You will receive a large amount of [ICON_GOLD] Gold [COLOR_YELLOW]equal to that gained over the last ' || (SELECT BaseGoldTurnsToCount FROM Units WHERE Type = 'UNIT_MERCHANT') ||  ' turns, and an instant "We Love the King Day" in all owned cities for ' || (SELECT BaseWLTKDTurns FROM Units WHERE Type = 'UNIT_MERCHANT') ||' turns. Every Town you''ve created and own increases the Gold and length of the "We Love the King Day" by ' || (SELECT Amount FROM Unit_ScalingFromOwnedImprovements WHERE ImprovementType = 'IMPROVEMENT_CUSTOMS_HOUSE' AND UnitType = 'UNIT_MERCHANT') || '%.[ENDCOLOR]'
...
UPDATE Language_en_US
SET Text = '[COLOR_CYAN]Special Improvement: Manufactory[ENDCOLOR][NEWLINE]You can expend a Great Engineer to create a Manufactory. [COLOR_YELLOW]When placed on a Strategic or Luxury Resource, the Manufactory connects it and provides an additional copy[ENDCOLOR] and, while worked, it grants huge amounts of [ICON_PRODUCTION] Production.[NEWLINE][NEWLINE][COLOR_CYAN]Special Ability: {TXT_KEY_MISSION_HURRY_PRODUCTION}[ENDCOLOR][NEWLINE]You can expend a Great Engineer to hurry production on the City''s current effort. [COLOR_YELLOW]The amount of Production is equal to ' || (SELECT BaseProductionTurnsToCount FROM Units WHERE Type = 'UNIT_ENGINEER') || ' turns of the best City''s average recent output. Every owned Manufactory you''ve created and own increases it by ' || (SELECT Amount FROM Unit_ScalingFromOwnedImprovements WHERE ImprovementType = 'IMPROVEMENT_MANUFACTORY' AND UnitType = 'UNIT_ENGINEER') || '%.[ENDCOLOR]'
...
UPDATE Language_en_US
SET Text = 'Agents stationed in the Cities of Major Civilizations can spend Network Points to conduct Spy Missions. There are several tiers of Missions with increasing power, culminating in the most expensive tier which includes the powerful "Steal a Technology" Mission.[NEWLINE][NEWLINE]Many Missions have requirements of the City (and/or Civilization) in which they are conducted. For example "{TXT_KEY_ESPIONAGE_EVENT_CHOICE_STEAL_FROM_TREASURY}" can only be performed in a [ICON_CAPITAL] Capital City.[NEWLINE][NEWLINE]There is an internal cooldown of 15 turns (equal in length to the time between Rigging attempts, see below) for a Spy before they can perform another Mission. Further, some sets of Missions share a cooldown that prevents multiple Civilizations using them back-to-back; the groups of missions that share cooldowns are listed in the page on {TXT_KEY_CONCEPT_ESPIONAGE_COUNTER_INTEL_TOPIC}, below.[NEWLINE][NEWLINE]Some Buildings and Wonders reward Yields when a Spy Mission is conducted by one of your Agents. Espionage can therefore be an engine for progress both from the effects of the Missions themselves, as well as the Yields that are earned in the process.'
...
UPDATE Language_en_US
SET Text = 'An important concept in Espionage are National Wonders. Starting with the Scrivener''s Office at Writing, and ending with the National Intelligence Agency at Computers, these are the main way you will receive [ICON_SPY_POINT] Spy Points. Not only that, these are some of the places you will receive yields from [ICON_PUPPET] Rigging Elections and conducting [ICON_SPY] Spy Missions.[NEWLINE][NEWLINE]Furthermore, the [ICON_CIVIL_SERVANT] Civil Servant Slots available on National Wonders are the some of the only ways you can increase [ICON_DIPLOMAT] Great Diplomat production, which is key if you want to maximise your [ICON_SPY_POINT] Spy Points and fully flex your Espionage on the world stage!'
...
UPDATE Language_en_US
SET Text = '[COLOR_YELLOW]Espionage[ENDCOLOR]'
WHERE Tag = 'TXT_KEY_GAME_CONCEPT_SECTION_22';
UPDATE Language_en_US
SET Text = '[COLOR_YELLOW]Religion[ENDCOLOR]'
WHERE Tag = 'TXT_KEY_GAME_CONCEPT_SECTION_23';
