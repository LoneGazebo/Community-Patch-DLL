...
-- 	Base Value of Test - Modifier to tech % cost. 1.25 is default.
	INSERT INTO Defines (
	Name, Value)
	SELECT 'BALANCE_HAPPINESS_TECH_BASE_MODIFIER', '1.25'
	WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_CITY_HAPPINESS' AND Value= 1 );

-- Base increase of threshold values based on # of citizens in cities you own. Modifier increases as cities grow. 90 is default. (Function is 'Need + ((CityPop x Value (90))/100)'. This value is added to City as a % modifier to needs.)

	INSERT INTO Defines (
	Name, Value)
	SELECT 'BALANCE_HAPPINESS_BASE_CITY_COUNT_MULTIPLIER', '90'
	WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type='COMMUNITY_CORE_BALANCE_CITY_HAPPINESS' AND Value= 1 );
